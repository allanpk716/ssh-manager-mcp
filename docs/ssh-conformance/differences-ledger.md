...
| Output truncation (> 1 MiB) | **not implemented in the broker today** (planned in spec §6; `ExecResult` is currently `{Stdout, Stderr, ExitCode, TimedOut}` with no truncation field) | no truncation | **Not a current difference** — the broker passes large output through unmodified. The §13.2 `large-output` subtest uses ~9 KiB, well under any plausible threshold. If a future `truncated` field is added it will be broker-specific and excluded from the differential. |
