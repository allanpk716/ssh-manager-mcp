...
| `relay_file` (server↔server large-file relay) | broker feature: chunked streaming relay THROUGH the broker's memory (source = a remote server or the broker's own disk, destination = always a remote server), resumable via the destination-side `<to_path>.sshmgr-partial` + manifest (re-running with the same paths completes only the missing chunks; `fresh=true` restarts), background-task trio (`task_id` / `exec_output` / `exec_stop`), metadata-only results (file bytes never enter the agent's context); DESTINATION sftp must support `posix-rename@openssh.com` (OpenSSH-family hard dependency, probed fail-closed at task start) | `scp serverA:file serverB:path` approximates the shape but is **not** equivalent — it is two DIRECT connections requiring the operator's credentials on both ends; the broker proxies with vault-held credentials the agent never sees, adds resumability anchored on the destination, and never exposes a credential-bearing client | **Broker-specific** — deliberately **excluded from the §13.2 differential** (no `ssh`-binary counterpart with the broker's credential semantics). |
...
agents see only the broker's twelve-tool surface — `list_servers` / `exec_command`
/ `download_file` / `upload_file` / `upload_content` / `exec_context` / `forward_port` /
`close_port` plus the background trio (`exec_background` / `exec_output` /
`exec_stop`) plus `relay_file` — with no
