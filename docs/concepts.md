...
└────────────────────┬────────────────┬──────────┼──────┼──────────────┘
                     │                │          │      │
        在线远程 MCP  │   project token（阀门）    │      │  设备码（水管钥匙）
      （.mcp.json 指 │                          │      │  + 服务器指纹（防伪封条，
       serve URL）   │                          ▼      ▼    首次连接必须核对）
                     │                ┌──────────────────────────┐
                     │                │ GET /snapshot（整仓快照）  │
                     │                └────────────┬─────────────┘
...
client 侧配置连接时，设备码与指纹两种填法完全等价，任选其一：

1. **分开填**：设备码一栏填裸码，指纹（pin）一栏填 `sha256:...`（向导表单 / `--pin` / 环境变量 `SSHMGR_SERVE_PIN` 均可）。
...
3. **client 机**跑 `ssh-manager tui` 进向导（填 server 地址 / 设备码 / 指纹，两种形态均可）或直接：
   ```bash
