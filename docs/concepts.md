...
└────────────────────┬────────────────┬──────────┼──────┼──────────────┘
   ...
   （project token 作   │                │          │      │  （设备码作 Authorization，
    阀门，逐命令把关）    │                │          ▼      ▼   指纹钉死 TLS）
   ┌.mcp.json 指→──────┐│                │  ┌──────────────────────────┐
   │ serve URL（在线）   ││                │  │ GET /snapshot（授权集快照）│
   └────────┬─────────┘│                │  └────────────┬─────────────┘
            │          │（离线模式：token 不出本机，              │
            │          │  在 cache.bin 里本地验证）              │
            ▼          ▼          ▼                             ▼
...
│    永不互认（project token 拿不到快照，设备码当不了 agent 钥匙）     │
│  ⚠ 设备码拉到的 = 它绑定 profile 的那箱（Plan 39 按授权裁剪）      │
└────────────────────────────────────────────────────────────────────┘
```
...
| **设备码** | **水管钥匙（限一辆车）** | 允许一台 client 机通过"水管"拉走**它绑定 profile 的那份**加密快照（cache.bin，Plan 39 起按授权裁剪——未授权服务器及其凭据不出服务器）；一台 client 一枚、绑一个 profile，吊销粒度=单机——吊销后该机**回连即销毁**本地 cache（≤30min 内撞上，四件全毁+隔离痕迹），永离线失窃根治仍=轮换服务器凭据 |
