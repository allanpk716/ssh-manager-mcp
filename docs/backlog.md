...
- ~~**doctor 多实例感知（维持登记，仍跟随 Plan 38-doctor 体系，不入 Plan 40 批1/批2）**：doctor 感知命名实例（枚举全部实例诊断）——目前不感知：仅命名实例的机器 doctor 的 client-cache 检查会报"cache 缺失"（roles 已修为 client；不静默但属误报）。~~ **已落地（2026-08-27, branch `doctor-multi-instance`）**：`checkClientCacheInstances` 枚举全部命名实例逐实例 sidecar 矩阵诊断（DEK 缺 FAIL / auth 缺 WARN / MAX_OFFLINE 到龄 WARN「下次使用即自毁」/ 空 slot INFO）；默认行误报修正（仅命名实例机器降级 INFO，全空机器维持 FAIL）；单槽覆盖（SSHMGR_CACHE_DIR/SSHMGR_CACHE_DEK）下整组跳过（一行 INFO）；顺带补齐 doctorEnvSeams 漏登记的 SSHMGR_CACHE_DEK_DIR / SSHMGR_CACHE_MAX_OFFLINE 两 seam。
