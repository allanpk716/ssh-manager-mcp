...
2. ~~隧道回收活动感知~~ → 已并入 #15。
4. ~~受控监听地址~~ → 已并入 #15。
5. **doctor serve 探活二期**（绿/黄/红语义）——现状：doctor 首版只做本机自检（P4，独立 plan）。设计草案已存在（2026-08-28，`docs/superpowers/specs/2026-08-28-plan-42-doctor-serve-probe-design.md`——编号勘误见文头注；其实例枚举构件已随 2026-09-15 抢救合并落地，见「Plan 40 多实例」节 doctor 条）。
