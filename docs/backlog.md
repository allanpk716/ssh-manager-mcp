...
12. **profile 删除功能（TUI+CLI 双缺）**——现状：`profiles` 只有 add/ls/grant，无法删除 profile。需先定级联语义：有项目绑定的 profile 删除时**拒绝**（报项目名）还是**级联断绑**（project.ProfileID 置空/连带处理）；store 需要对应 DeleteProfile 变体 + 测试。
13. **grant 全量同步语义（3B）**——现状：v0.8.3 已做预勾选（3A，纯可见性），提交仍是 INSERT OR IGNORE 只增不减——取消勾选**不会**移除已有授权（表单标题已如实声明）。若要"勾选集=授权集"需 store 增加移除路径（按 profile 删未勾选项）+ 权限语义评审（撤销对 agent 的可见性影响同 revoke 授权面）。
