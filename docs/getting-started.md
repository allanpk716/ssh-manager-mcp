...
#   选对应平台，解压后把 sshmgr 放到 PATH 里

# 方式 B：本地编译（需要 Go）
go build -o sshmgr ./cmd/sshmgr

# 方式 C：go install（装到 $GOPATH/bin）
go install ./cmd/sshmgr
```

验证：

```bash
sshmgr version     # 应打印版本号（本地编译是 dev）
sshmgr --help      # 列出所有子命令
```

> 后文示例都假设 `sshmgr` 在 `PATH` 里。如果没有，用绝对路径替换，例如 `./sshmgr ...`。
> **Windows 用户**：二进制名是 `sshmgr.exe`；在 `.mcp.json` 里写 `"command": "sshmgr"` 时，要确保它确实在 PATH，或干脆写绝对路径（见 Step 5 的 Windows 提示）。
...
> 💡 不想背命令行？**Step 2-4（加服务器/建 profile/发 token）都可以在 TUI 主控台里完成**：`sshmgr tui`（全屏界面，凭据掩码录入、token 一次性展示——见 [README「TUI 主控台」](../README.md#tui-主控台sshmgr-tui)；概念分不清时翻 [concepts.md](./concepts.md)）。
...
验证：`sshmgr servers ls` 应能看到 `gpu`。

> 💡 **手上已有一份 `~/.ssh/config`？不用逐台敲。** 一条命令批量导入（TUI 主控台服务器页按 `i` 也一样）：
>
> ```bash
> sshmgr servers import --dry-run          # 先看会发生什么（什么都不写）
> sshmgr servers import --profile team-a   # 确认无误再真导入，顺手 grant 进 profile
> ```
>
...
sshmgr profiles grant team-a gpu          # 可一次 grant 多台：... team-a gpu db web
```

预期输出分别是 `created profile team-a id=...` 和 `granted 1 server(s) to team-a`。

`sshmgr profiles ls` 能看到每个 profile 里 grant 了几台 server。
...
.mcp.json snippet:
{"mcpServers":{"ssh":{"command":"sshmgr","args":["mcp"],"env":{"SSHMGR_TOKEN":"eyJ..."}}}}
```

- **马上把这段记下来**（密码管理器里）。token 之后再也查不到明文（库里只存它的 hash）。
- 如果你弄丢了，别慌：`sshmgr projects rotate my-agent` 会换发一个新 token（旧 token 立刻失效），详见 [agent-access.md](./agent-access.md)。
...
- **Windows**：`sshmgr` 必须能被 Claude Code 解析到。最稳妥是写**绝对路径**，例如 `"command": "C:\\path\\to\\sshmgr.exe"`（JSON 里反斜杠要转义成 `\\`）。
- **别提交 git（公开仓库尤其致命）**：`.mcp.json` 含**活 token**，必须加进 `.gitignore`，绝不能提交进 git 仓库。（Claude Code 首次加载项目级 MCP 会弹确认。）
- 也可以用命令行注册到**用户级**（所有项目可见）：
  ```bash
  claude mcp add ssh sshmgr -e SSHMGR_TOKEN=<TOKEN> -- mcp
  ```
...
开机 → 启动 Claude Code（它读 .mcp.json）
     → 你让它用 ssh 工具
     → Claude Code spawn 子进程: sshmgr mcp（token 经 env SSHMGR_TOKEN）
...
1. **`sshmgr` 二进制在 PATH**，或 `.mcp.json` 里写了**绝对路径**（Windows 强烈建议绝对路径，见 Step 5）。
2. **`.mcp.json` 已配好**（项目级或用户级都行），并且别提交 git。
3. **master.key 文件已存在**（跑过一次 `sshmgr unlock`）——这是"重启零干预"的根本保证。MCP 子进程能自己读到 master.key，**你连 `unlock` 都不用再跑**（见上文"日常"：`unlock` 只需第一次跑）。
...
```bash
sshmgr servers ls          # 能列出 = store.db + master key 都健康（数据不依赖任何在跑的进程）
sshmgr projects ls         # status=active
SSHMGR_TOKEN=<你的token> sshmgr mcp   # 手动跑客户端会跑的那条命令；不报 vault locked、停在等 stdin = 子进程能独立起来（Ctrl+C 退出）
```
...
如果你要**多机共享 vault**（笔记本 + 台式机共用一份清单），需要在 VLAN 一台服务器上常驻 `sshmgr serve`。Plan 16 起，`serve install` 用 [`github.com/kardianos/service`](https://github.com/kardianos/service) **跨平台注册**系统服务：
...
如果你偏好用第三方服务包装器而非内置的 `serve install`（例如已有统一的服务管理工具链），手动包也行——只要包装的是同一条 `sshmgr serve --addr ...` 命令：

- **Windows（NSSM）**：
  ```powershell
  nssm install sshmgr-serve "C:\path\to\sshmgr.exe" "serve" "--addr" "0.0.0.0:7878" "--tls-cert" "cert.pem" "--tls-key" "key.pem"
  nssm set sshmgr-serve AppDirectory "C:\path\to"
  nssm set sshmgr-serve AppStdout "C:\ProgramData\ssh-manager\serve.log"
  nssm set sshmgr-serve AppStderr "C:\ProgramData\ssh-manager\serve.log"
  nssm start sshmgr-serve
  ```
  注意：NSSM 默认以 `LocalSystem` 跑（与内置 install 一致）；`master.key.plain` 的 ACL 必须让 `LocalSystem` 可读（默认如此）。

- **Linux（systemd 手动 unit）**：
  ```ini
  # /etc/systemd/system/sshmgr-serve.service
  [Unit]
  Description=sshmgr MCP server (serve mode)
  After=network.target

  [Service]
  ExecStart=/usr/local/bin/sshmgr serve --addr 0.0.0.0:7878 \
            --tls-cert /etc/sshmgr/cert.pem --tls-key /etc/sshmgr/key.pem
  Restart=on-failure
  ...
  ```
  ```bash
  systemctl daemon-reload && systemctl enable --now sshmgr-serve
  ```
...
- **macOS（launchd 手动 plist）**：
  ...
  <dict>
      <key>Label</key>
      <string>com.sshmgr.serve</string>
      ...
      <array>
          ...
          <string>/etc/sshmgr/cert.pem</string>
          <string>--tls-key</string>
          <string>/etc/sshmgr/key.pem</string>
      </array>
      <key>RunAtLoad</key>
      <true/>
      <key>KeepAlive</key>
  ...
  ```
  ```bash
  sudo launchctl load -w /Library/LaunchDaemons/com.sshmgr.serve.plist
  ```
...
| 现象 | 原因 / 处理 |
|---|---|
| `vault locked: run sshmgr unlock` | 没跑过 `unlock`（`master.key.plain` 还没生成）；或非特权进程建不了固定路径目录（Windows 需 admin / Unix 需 root 先 `serve install` 建目录）。 |
...
| `master key not found: run 'sshmgr unlock' in an interactive session first`（`serve install`） | 还没 `unlock` 就装 service。先 admin 跑 `unlock` 生成 master.key，再 `serve install`。 |
| agent 报 `server is not in your profile` | 你让它操作的 server 不在它 project 绑定的 profile 里。用 `sshmgr projects show <name>` 核对。 |
| agent 看不到 sudo / `has_sudo=false` | 加服务器时没给 `--sudo-password`。用 `sshmgr servers edit <name> --sudo-password '...'` 补上。 |
...
| Windows 上想替换 `sshmgr.exe` 报"文件被占用" | serve 常驻进程锁着二进制。先 `sshmgr serve uninstall`（停 service + 杀 serve）→ 替换 exe → 再 `serve install`。见 [multi-machine.md](./multi-machine.md)。 |
