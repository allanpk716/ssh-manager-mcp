...
录入新目标前先想清楚一个位置问题：首次信任一台目标（把它的主机密钥锚进权威 vault）需要**到目标的网络通路** + **可写的权威存储**，而这两样常在不同机器上。

- **谁能连到那台目标？** broker 自己可达 → 首次连接照旧自动落锚，无需任何操作。只有某台工作机可达（broker 对目标网段没有路由，如工控网段只接了笔记本一块网卡）→ 该机 agent 首连时自动**锚定转发**：broker 在线时锚经审计落库、首连一次成功，零仪式。
- **首连时 broker 不可达怎么办？** 首连失败、不留任何状态，错误文案带呈现的指纹和两条补救路径：等回网后重试（锚自动转发），或 owner 拿指纹一条命令**带外锚定**：`sshmgr servers pin-hostkey <名> --fingerprint SHA256:…`（指纹可从目标机屏幕或 `ssh-keyscan <目标>` 输出取得）。

（多机 agent 无写权限的设计不变——锚定转发不是本地可写层，锚始终落在权威 vault 且带审计。行为细节、清毒时序、锚残留与混布窗口见 [multi-machine.md「主机密钥锚定」](./multi-machine.md#主机密钥锚定锚定转发与带外锚定plan-48)。）
