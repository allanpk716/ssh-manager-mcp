...
把上面的片段写进 `~/.mcp.json`（或项目级 `.mcp.json`），重启 Claude Code。现在 agent 有了 10 个 SSH 工具（`list_servers` / `exec_command` / `download_file` / `upload_file` / `forward_port` / `close_port` / 后台三件套 `exec_background` / `exec_output` / `exec_stop` / `upload_content`），范围仅限 `team-a` profile 里你授权的服务器。
