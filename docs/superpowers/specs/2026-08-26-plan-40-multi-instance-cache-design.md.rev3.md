...
>
> **rev2（2026-08-26）**：二轮外部评审确认的 9 条问题全部吸收（对象 = rev1）。关键修订：① §2.4 身份门禁语义完整化——生效条件改为"默认目录 `cache.bin` 存在"；重绑定 = 清除 cache 材料后重新 enroll；auth 写序钉死；② §2.4/§5 老 serve（响应头缺失）× 无 `--instance` 行为钉死（门禁跳过 + 不归位 + 提示）；③ §2.1 casefold 完整化——查重扩全历史（含 revoked）、入事务、存量碰撞启动检测、client 物理碰撞检测、保留名判定改首段（实测 `con.foo`/`COM1.x` 过全名等值但 `MkdirAll` 必败）；④ §2.2 env × flag 互斥报错；⑤ §3 env 在场写 config → WARNING；⑥ §9 登记（跨进程 casefold 窗口、TUI 表单预警归二批）。
>
> **rev3（2026-08-26，终版，owner 特批书面级收口——三轮评审轨迹 11→9→7 单调收敛，末轮 7 条全为文字级/登记级）**：① **§2.4 门禁"存量未登记"分支修正（双盲共识高条）**——rev2 把"meta 可解析但 `device_name` 空串"（**所有存量机器的必然形态**——字段随本设计新增；老 serve 拓扑首次 enroll 同样产出空值）与"meta 不可解析"混入同一拒绝分支，会使每台存量机器首次 pull 被拒、违背 §5 零迁移承诺；修正为三分支：可解析且非空 → 比对拒/放行；**可解析但空串（存量未登记）→ 放行并随本次 pull 补记身份（补记后门禁生效，补记前异码敞口维持现状级）**；不可解析但 bin 在 → fail-closed 拒（真异常态）。② §2.1 存量启动检测扩为 casefold 碰撞 **+ 白名单校验**（自由文本时代的非法 name 一并拦）。③ §2.5 钉死"默认无 cache 且有实例 → 报错列实例"**仅适用于 `mcp --cache`**，`cache status` 无 `--instance` 恒为列全部（§2.6 语义）——消除两节矛盾。④ §2.1 物理碰撞检测补半写态：目标实例目录 bin 存在且无可读匹配 meta → fail-closed 拒 + 清理路径。⑤ §2.4 换码 runbook 清理范围拍板：**语义 = 按目录/槽位**——`cache.config.json` 保留继承（MAX_OFFLINE 是实例策略非设备码属性，runbook 写明），**`quarantine/` 并入清除清单**（旧身份加密材料不留，终版四件套 = auth + bin + meta + quarantine/）。⑥ §9.5 测试指引改写（多实例测试走 `--instance` 不设 DEK env，消除与 §2.2 互斥的冲突）。⑦ §9 补登 auth-only 边缘（bin 缺失时门禁不生效，lazy pull 在默认目录建材料——首次 pull 补记后自然闭合）。
>
...
- **name 白名单（含 Windows 文件系统语义）**：
  ...
  - **大小写折叠唯一性（全历史 + 事务内 + 存量检测）**：server 端 `cache-tokens add` / `bind` 按 casefold（ASCII 小写）查重，**范围 = 全部历史行（含 revoked）**——revoke 后重发大小写变体会与残留实例目录/DEK 碰撞，必须终身占用；查重与 INSERT **同事务**（store 连接 `MaxOpenConns(1)` 已使进程内天然串行，事务化是跨进程双开的兜底——残余窗口见 §9.8）；**存量检测（serve 启动 fail-closed）**：扫描 cache_tokens 全部 active 行，发现 **casefold 碰撞或白名单不合规**（自由文本时代的非法 name：控制符/非法 Unicode/路径形态——会污染 `X-Sshmgr-Device-Name` 下发）任一 → **启动失败** + 引导 owner 手工改名（`cache-tokens revoke` + 新名 add）。
  - client 端 pull 消费 name 前做同款字符白名单 + 首段保留名校验（防御存量非法 name），非法 → 拒写盘，文案引导 owner 改名重发；**物理碰撞检测（含半写态）**：目标实例目录已存在且其 meta 身份 ≠ 本次 name → 拒绝写盘；**目录已存在但 `cache.bin` 在而 meta 缺失/不可读 → fail-closed 拒绝**（中断/半写残留不自动覆盖——清理路径 = 删除该实例目录后重 enroll，写入拒绝文案）。
...
- **env × flag 互斥**：`SSHMGR_CACHE_DIR` 或 `SSHMGR_CACHE_DEK` 显式设置**且** `--instance` 显式给出 → **报错**（fail-closed 互斥，冻结文案指明二者只能其一）——静默忽略 flag 正是本设计反对的"静默错位"（可能路由到错误 cache 或令多实例共享同一 DEK）。要走 env 的测试/迁移场景删掉 flag 即可。
...
`ct` 鉴权后已在手，零查询成本。**非安全边界**（name 非秘密；pinned TLS 已防篡改）——仅作路由与一致性校验数据。老 client 忽略之（向前兼容）。存量非法 name 在 §2.1 启动检测处拦（不会到达下发）。

### 2.4 pull 写入规则（`cache pull [--instance <name>]`）——rev3 门禁三分支 + 换码 runbook 终版
...
- **默认实例身份门禁（rev3 三分支终版）**：
  ...
  - **三分支判定规则**（`DoPull` 成功路径在**任何写盘之前**（含 bin）执行）：
    1. meta **可解析且 `device_name` 非空**、≠ 本次响应头 name → **fail-closed 拒绝**，冻结文案引导三选一（这是不同设备码：第二个实例请用 `--instance <name>`；要更换默认实例的设备码：按下方换码 runbook 清材料重 enroll；owner 核对发码是否张冠李戴）。
    2. meta **可解析但 `device_name` 为空串**（存量未登记——**所有存量机器升级后的必然形态**，老 serve 拓扑首次 enroll 亦产出空值；字段随本设计新增）→ **放行，并随本次 pull 补记身份**（写盘时 meta 记录本次 name）。补记前异码敞口维持现状级（本来就在），补记后门禁即刻生效——存量机器零迁移零感知，§5 承诺成立。
    3. meta **不可解析（缺失/损坏）但 bin 存在** → **fail-closed 拒绝**（真异常态——不以不可读记录为放行依据；拒绝文案引导按换码 runbook 清材料重 enroll）。
  - **换码 runbook（rev3 终版，语义 = 按目录/槽位）**：更换默认实例的设备码 = 清除默认目录 cache 材料**四件套**——`cache.auth.json` + `cache.bin` + `cache.meta.json` + **`quarantine/` 子目录**（旧身份的隔离材料不留）——后重新 enroll。**`cache.config.json` 保留继承**（MAX_OFFLINE 是实例/目录策略而非设备码属性——目录的离线时效不随换码变化；此继承语义为有意拍板，runbook 与文档写明）。
  - **`cache.auth.json` 写序钉死（防错位）**：所有 auth 写入必须位于 `DoPull` 成功**之后**——CLI pull 路径现状已如此（§0.11 核实：门禁拒绝即提前 return，auth 不动，写序属回归钉住）；TUI `[s]同步` 用已存 cred 不写 auth；**TUI 连接编辑表单换码**是唯一先写 auth 的路径（保存即写、之后同步才撞门禁）——其错位后果由门禁拒绝文案兜底（用户被引导按 runbook 清材料或用 `--instance`），表单内的**预防性警告**归第二批 TUI 改造。
  - 门禁在 `DoPull` 内一处接线，CLI/TUI/lazy 全路径覆盖。
- `--instance` 与响应头 name 的强一致校验同前（防 owner 发码张冠李戴——实例目录与授权错位）。

### 2.5 读取规则（`mcp --cache [--instance <name>]`）

- 无 `--instance` = 默认目录（现状）。**`mcp --cache` 在默认目录无 cache 且 `instances/` 下存在实例时 → 错误文案列出实例清单并指引 `--instance <name>`**（不自动猜——读到哪个实例必须显式，杜绝静默错位）。**本条仅适用于 `mcp --cache`；`cache status` 无 `--instance` 恒为列全部实例（§2.6 语义）——status 是列表命令，不受"不自动猜"约束（rev3 钉死，消除与 §2.6 的矛盾）。**
...
- 无 `--instance`：列**全部**实例——默认实例一行 + `instances/*/` 每行（含默认目录无 cache 而 instances 有货的情形——纯列表，不报错）。字段：实例名 / 设备码 name（meta `device_name`）/ profile（仅 `scoped=true` 时显示，Plan 39 溯源纪律）/ servers / creds / age / 锚状态 / quarantine 概要。
...
- **env 共存提示**：`pull --max-offline` 执行时若 `SSHMGR_CACHE_MAX_OFFLINE` env 在场 → 输出一行 WARNING（"env 在场，本次写入的 config 在 env 清除前不生效"）——防止用户以为持久化已生效。
...
- **与换码 runbook 的关系（rev3）**：config 属目录/槽位，换码不清除（§2.4 runbook）。
...
- **实例间隔离**：① DEK per-instance——A 实例目录连同其 DEK 单独泄露，解不开 B 实例 cache.bin（分离布局下"目录被同步工具带走"不含 DEK）；② token×cache 交叉 fail-closed——B 的 project token 在 A 实例 spawn 即验证失败（既有机制：裁剪 snapshot 只含 A profile 的 projects 行，MCP 层授权过滤本来就对）；③ name 大小写折叠唯一性（全历史 + 存量启动检测，§2.1）——封死两个授权映射同一目录/DEK 的错配（含 revoke 后重发变体、存量遗留与自由文本时代非法名）。
- **路径穿越**：name 白名单双端校验；`instances/<name>` 的 name 恒过白名单才进 `filepath.Join`。
- **错配防护**：`--instance` 与响应头 name 强一致校验；默认实例 `device_name` 门禁（§2.4 三分支，含存量补记与真损坏拒载的区分）——异码覆盖在写盘前被拒；client 端物理碰撞检测（§2.1 含半写态）。
- **残余（非新增）**：同机恶意进程可读所有实例材料（文件 ACL/DPAPI 不挡同机）——与单实例现状同级。

## 5. 兼容性

- **存量单实例机器**：零迁移零行为变化（默认实例 = 现路径；无 flag 的 pull/mcp/status 全走默认目录；门禁对存量空 `device_name` 走补记分支（§2.4），首次 pull 无感补记身份）。
- **新 serve + 老 client**：多一个响应头，老 client 忽略——零影响。
- **老 serve + 新 client**：`--instance` 拒（提示升 serve，§2.4）；无 flag 路径 = **现状行为 + 门禁跳过（无 name 可比）+ 不归位落默认目录 + 升级提示**（§2.4 表格钉死）。升级顺序铁律（client 先 serve 后）继续成立并在 compat-matrix 强调。
- **存量大小写碰撞/非法 name（升级前 BINARY/自由文本时代遗留）**：serve 启动 fail-closed（§2.1 存量检测）——owner 改名后恢复。
...
4. name 白名单：server `add` 拒非法（含**大小写折叠碰撞**：对 active 与 **revoked** 各一例、`agentA` vs 已有 `AGENTA`、**首段保留名** `con.foo`/`COM1.x`、全名保留名、尾点）；client 端非法 name 头拒写（含 `../` 形态）；**serve 启动对存量 active 折叠碰撞/非法 name fail-closed**；**client 物理碰撞**（目录在而身份不同 → 拒；**bin 在而 meta 不可读 → 拒 + 清理路径文案**）。
...
8. **默认实例身份门禁矩阵（rev3 三分支）**：① 已有 `device_name=X` 的默认目录，用 Y 码 pull → 拒绝（**bin/meta/auth 三者字节均不变**）；② **bin 在 + meta 可解析 + `device_name` 空（存量升级形态）→ 放行且 meta 补记本次 name**（零迁移回归钉）；③ **bin 在 + meta 缺失/损坏 → 拒绝**（真异常态）；④ 真空（无 bin 无 meta）→ 放行；⑤ **换码 runbook：清四件套（auth+bin+meta+quarantine/）后重 enroll → 门禁不触发、config.json 保留继承**；⑥ 仅删 auth.json（bin/meta 在）→ 门禁仍按三分支判定（文案引导 runbook）。
...
11. `clear` 清全部实例 + DEK 变体；`roles` 判定：仅命名实例的机器判为 client；**`cache status` 无 --instance 列全部（含默认目录空而 instances 有货）**。
...
- `docs/multi-machine.md`：多实例章节（enroll 双 agent 流程、`--instance` 用法、失窃响应 = **吊销设备码切断增量 + 轮换受影响凭据**、吊销纪律：快速断 agent = 先吊 device code（下次 pull 销毁该实例 cache）再吊 project token（等 pull/过期）、**默认实例换码 runbook（清四件套重 enroll，config 保留继承语义写明）**、过渡期应急纪律 §1.3）。
...
- `docs/compat-matrix.md`：§5 两行 + 存量大小写碰撞/非法 name 的启动失败行。
...
- **第一批**：多实例 CLI 闭环——`CachePathsFor`/`CacheDekPathFor`/`DekProvider` 参数化 + 全消费面接线（§2.2 含 env×flag 互斥）、name 白名单双端含首段保留名/全历史折叠/存量启动检测（碰撞+非法名）/物理碰撞检测含半写态（§2.1）、server name 头（§2.3）、pull 写入规则含**默认实例身份门禁三分支**（§2.4 一批列）、mcp/status 读取规则（§2.5-2.6）、clear + roles（§2.7）、MAX_OFFLINE 持久化（§3）、文档联动（§7 首批部分）。**首批 enroll 保持现状落默认目录（自动归位不在此批）。**
...
5. **测试指引（rev3 改写）**：多实例测试走 `--instance` flag、**不设** `SSHMGR_CACHE_DIR`/`SSHMGR_CACHE_DEK` env（§2.2 互斥规则下二者不可同用）；DEK env 仅用于单实例/默认实例场景的临时域隔离。
...
8. **跨进程 casefold 双插窗口**：查重已入事务 + 进程内 `MaxOpenConns(1)` 天然串行；两个独立 ssh-manager 进程**恰好同时** add 互为大小写变体的理论窗口仍存在——owner 单人操作面下接受为残余（发生时 client 端物理碰撞检测兜底拦截）。
9. **TUI 表单换码预防性警告归二批**（§2.8）：第一批该路径的错位由门禁拒绝文案兜底（用户被引导按 runbook 清材料或用 `--instance`），体验欠佳但非静默。
10. **auth-only 边缘（rev3 登记）**：bin 缺失 + `cache.auth.json` 在（TUI 表单先写 auth 的固有形态）时门禁不生效（生效条件是 bin 存在），lazy pull 将在默认目录建材料——用户本意若是命名实例则属误置；首次 pull 补记身份后门禁即闭合该窗口。接受为边缘残余（表单预防性警告二批缓解）。
