...
- **单测**(零新依赖,httptest+注入 seam):
  ...
  - **事务性**(临时副本+rename 失败注入,单测可跑,不限平台):校验失败→目标字节不变;写入失败→回滚→目标字节不变;**staged version 自检失败(假输出/超时[注入 exec seam])→拒绝**;**--file 路径 staged 输出=目标版本断言**;**回滚 double-fault→普通失败码+错误消息携带可执行恢复命令**;**启动自愈分支**(self 缺失+代际 .old 存在→恢复;**非 TTY+--yes 遇自愈→报错退出**;执行路径带 .old 后缀+canonical 缺失→触发);**代际 .old:两代备份并存,回滚取最新**
  ...
  - 服务检测(seam 替身):旧名存在→迁移块+中止;**双服务并存→中止**;**failed 态→按已装放行(不封死崩溃循环机的更新)**;**探测机制错误→fail-closed**(**无"跳过"分支——勘误1:哨兵生产不可达**);**路径预检(替换前)**:读不到→中止;不一致→中止打印两端;**比较规范化**(大小写/符号链接口径)
