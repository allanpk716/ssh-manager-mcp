...
- **三件套双屏显示（冻结）**：两侧各显示同一行：`<name> @ <target_url> SAS <6位>`。
  ...
  > 勘误（2026-09-01，**撤销上条**，三件套双屏按冻结原文恢复）：「无法计算」只对批准进程**自行**计算成立——serve 在 enroll 时即持有临时私钥与请求内的 client_pub，当场派生 SAS 落 `pairing_pending.sas` 列（跨进程共享总线，本节开头 F8 形态），批准界面（TUI/CLI/批2 Web）直读真值同屏显示。私钥/K_ack/K_creds 仍只在 serve 进程内存；audit 白名单不变（§3.3-8，SAS 值照旧不落 audit——pending 行是批准工作面，audit 是可导出日志面，暴露面不同）。行缺 SAS（旧版 serve 错配）→ 批准面 ⚠ 警示并建议拒绝，**不静默回退两件套对照**（name/target_url 是未认证的 enroll 输入，MITM 转发时天然一致，两件对照对 MITM 无感）。附带加固：enroll 即做 ECDH，低阶/垃圾 client_pub 提前 400（原拖到 finish 才暴露）。
