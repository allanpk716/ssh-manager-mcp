# Plan 43 — doctor serve 探活二期（绿/黄/红语义）设计 spec（rev0 基稿）

> **编号勘误与修订链抢救（2026-09-16 收尾注）**：本基稿写于 2026-08-28，原名「Plan 42」——该编号其后已归属模式缩减+发现配对 usability plan（同日晚些定案），本稿遂改挂 Plan 43。其后的两轮异构盲评收敛（第一轮 13 条反馈全闭环、复审轮 8 条反馈全闭环）当时未及提交，2026-09-16 从被弃 worktree 抢救——**最新定稿 = 同目录 `.rev2.1.md`（rev2 技术内容零改动，rev2.1 为编号适配，待 owner 审）**，本基稿仅作修订链的历史锚保留。本稿非 usability Plan 42 的实现。
