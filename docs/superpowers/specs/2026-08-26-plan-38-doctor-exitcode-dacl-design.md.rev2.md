...
> backlog P2 首批（doctor 系三连的前两个；#5 serve 探活二期是下一个独立 plan，依赖本 plan 的 #7 接线）。2026-08-26 brainstorming 已拍板、本文不重议的决策：**DACL 过松 → WARN（对称 Unix mode 0644→WARN 先例，钉死于 TestDoctorVaultStructural）**、**检测范围只 master.key（API 按 path 泛化，store.db/cache-dek.key/WAL 留 backlog 后续）**、**方案 A：store 深模块读 API（判定语义与 HardenACL 写侧同居）+ cli 侧 ExitCodeError 接口约定**；评审闭环中追加拍板：**白名单 read-capable 判定语义**、**信号 2（继承未断）降级 Detail 辅助**、**exit 2 帮助文本随 #5 上**。
>
> 本版 = **rev2**（第 2 轮评审闭环修订）。相对 rev1 的关键变更：**新增信号 4（owner 隐式提权）**——Windows 对象 owner 隐式持有 WRITE_DAC（微软文档证实），非白名单 owner 可改写 DACL 再读明文，白名单从「扫 allow ACE」补全为「ACE + owner」双面；**icacls fix 顺序修正**（/inheritance:r 必须先于 /remove——继承 ACE 不能直接移除，rev1 序列在宽父继承场景是无效指引）；矩阵① null-DACL 腿如实化（不宣称等效覆盖）；补 WRITE_DAC/WRITE_OWNER-only 与 GENERIC 位实证腿；ExitCodeFor 对绕过构造器的字面量兜底；UnexpectedReadGrantors 按 SID 去重排序；fix 占位符明确；§7 增属主语义登记。**GENERIC 位假设已实证**（2026-08-26 探针：存入 raw 0x80000000 读回 0x00120089，恒展开为 specific rights）。
...
type FileACLReport struct {
    ...
    OwnerUnexpected        bool     // owner ∉ 白名单（信号 4；owner 隐式 WRITE_DAC，见 §1.2）
}
...
### 1.2 判定语义（只查「过松」不查「过严」；白名单 + owner）
...
3. **白名单外主体带危险位的 allow ACE**：walk DACL，对每个 allow 类 ACE（`ACCESS_ALLOWED_ACE_HEADER` 型；**deny 型不算**——`Everyone:(deny)R` 是收紧不是暴露，事实 #9 的修正），若 trustee **不在白名单**且 mask 含危险位 → 记入 `UnexpectedReadGrantors`（SID 字符串形态，不做 LookupAccountSid 本地化解析）。
...
4. **owner 非白名单**：Windows 对象 owner **隐式持有 WRITE_DAC**（微软文档《Owner of a New Object》："An object's owner implicitly has WRITE_DAC access to the object"）——非白名单 owner 即使没有任何 allow ACE，也能改写 DACL 给自己授权后读明文。`GetNamedSecurityInfo` 加请求 `OWNER_SECURITY_INFORMATION`，读回 owner SID 比对白名单；非白名单 → `OwnerUnexpected`（Detail 呈现 `OwnerSID`）。写侧路径不改 owner（事实 #5），owner = 创建者用户，硬化态恒白名单内；owner 异常即文件被外部重属主。
...
`getDACLForTest` → `readDACL`（私有生产名）、`isDaclProtected` / `trusteeInACL` 移出 test-helper 注释块转正（名字本就中性）。`acl_windows_test.go` 全部调用点跟着改。**行为零变化，纯搬家+正名**——`InspectFileACL` 是它们之上的新生产入口（allow-ACE 过滤 + 白名单比对 + mask 过滤 + owner 判定的新逻辑落在 InspectFileACL 内）。
...
WARN Detail 模板（SIDs 为 `UnexpectedReadGrantors` 列表，逗号并列；owner 异常时追加第二句；命中时 !Protected 则追加括注）：

```
master.key present (32 bytes) but its DACL grants access to unexpected principals: <SIDs> — the plaintext key is protected by this ACL alone[ (inheritance also enabled)][; file owner is <OwnerSID> — the owner can rewrite the DACL]
```

WARN Fix（v1 指引无效——`/inheritance:r` 只移除继承 ACE；rev1 序列 `/remove` 先于断承亦无效——**继承 ACE 不能直接 remove**，微软文档与实证社区一致。正确顺序：**先断承清掉继承宽 ACE，再移除剩余显式宽 ACE，再重授**。SID 用 `*S-1-...` 星号形态（icacls 数字 SID 语法，防本地化账户名不解析）；owner 异常时先归位 owner）：

```
restore a restrictive DACL: [icacls <master.key> /setowner "Administrators", then] icacls <master.key> /inheritance:r /remove:g <SIDs...> /grant:r "SYSTEM:(F)" /grant:r "Administrators:(F)" /grant:r "<you>:(RC,R,W,D)" — replace <SIDs...> with the principals listed above (asterisk-prefixed SID form, e.g. *S-1-1-0) and <you> with your own account (find your SID via `whoami /user`)
```

（unlock 路线已核实不可行，事实 #11；`/grant:r` 替换语义、`/remove` 显式 ACE 移除、继承 ACE 不可直接 remove 均为微软文档/实证确认。）
...
// NewExitCodeError is the sanctioned constructor: code >= 1 and err != nil
...
// bypassed NewExitCodeError with a nonsense code (<1) falls back to 1 rather
// than exiting 0 on an error path.
...
（字段导出供 errors.As 与测试断言；构造器校验 + ExitCodeFor 兜底双防线——绕过构造器的字面量最多退化为 1，不会「错误但 exit 0」。）
...
| `TestInspectFileACL_TooLoose_Matrix` | ① **null DACL**（实现期 TDD 首步 = 先跑种法探针：`SetNamedSecurityInfo` 传 nil dacl；**可行**则强制断言 `DaclNull && TooLoose`；**不可行**则降级腿「显式 Everyone-allow + UNPROTECTED」只覆盖信号 3，信号 1 的 DaclNull 判定分支以解析层构造单测钉，§7 登记残差——不宣称等效覆盖）；② **宽父继承腿**（复刻事实 #14 实验：临时目录种宽可继承 DACL（Everyone-allow FullControl，`SUB_CONTAINERS_AND_OBJECTS_INHERIT`）→ 其下建子文件不硬化）→ 子文件 `UnexpectedReadGrantors` 含 Everyone SID、`TooLoose`；③ 显式种 Everyone-allow 读 ACE → 抓到；④ 显式种 BUILTIN\Users-allow 读 ACE → 抓到（非白名单任意 SID 代表）；⑤ 显式种 Everyone-allow **仅写** mask（WRITE_DATA，无 FILE_READ_DATA）→ **不**算危险（钉 mask 过滤）；⑥ 种 Everyone-**deny** 读 → 不算（allow-only 语义钉子）；⑦ 显式种 Everyone-allow **仅 WRITE_DAC** → 抓到（提权位正向）；⑧ 显式种 Everyone-allow **仅 WRITE_OWNER** → 抓到（提权位正向）；⑨ 种 raw `GENERIC_ALL`(0x80000000) allow ACE → 读回已展开（含 FILE_READ_DATA）→ 抓到（钉事实 #15 回归：展开假设破坏时此腿红） |
| `TestInspectFileACL_Owner` | 种白名单外 owner（`SetNamedSecurityInfo` 带 `OWNER_SECURITY_INFORMATION`，owner 设为 Everyone SID）→ `OwnerUnexpected && TooLoose` 且 `OwnerSID` 呈现；owner 归位 Administrators → 不触发 |
| 不存在路径 | 返回 error |
...
- **Windows-only 腿**（`runtime.GOOS == "windows"` guard，对齐 Case 3 的 Unix-only 模式）：
  ...
  - （廉价顺带）种 owner 异常 → WARN 含 owner 子句；
  - （廉价顺带）种 null DACL（若 §4.1 ① 探针证实可行）→ 同 WARN 通道。
...
- `ExitCodeFor` 四态：`NewExitCodeError(2, err)` → 2；普通 error → 1；nil → 0（防御分支）；`&ExitCodeError{Code: 0, Err: errors.New("x")}`（字面量绕过构造器）→ **1**（兜底钉子）。
...
- **null-DACL 强制测试未决**（§4.1 ①）：nil-dacl 种法探针留作实现期 TDD 首步；若证实不可种，信号 1 的覆盖降级为解析层构造单测 + 代码审查，登记不宣称端到端。
