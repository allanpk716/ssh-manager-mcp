...
> 本版为第二版（2026-08-21 交叉评审收敛修订）：吸收 11 项必改——配额驱逐策略（满员自动驱逐最旧终态项）、failed 态防御性清洗、唤醒原语、Snapshot 三分支游标钉死与深拷贝、终态即关连接、revoke 机制声明+测试钉住、exec_stop 立即返回、stop/自然退出竞态互斥、offset 非负校验、env seam 非正数非法、内存上界点明。两项经实验证伪排除：终态排空窗口（自然退出 10/10、强杀形态 20/20 实测 Run 返回后 writer 零新增——库 Wait 语义已保证排空）；运行期错误含地址（客户端拆连/RST/FIN 三种死法实测全为 `wait: remote command exited without exit status or exit signal`，零地址形态）。
...
- **每任务独占一条 ssh.Client**（forward_port 同姿势）：`exec_background` 同步走完 exec_command 的全部门链（profile 门 → GetServer → AuthForServer → HostKeyTOFU → Connect，含 Plan 31 错误清洗——连接错误当场返回给 agent），成功后才入表；**Client 生死全归 TaskManager，session 返回即关 Client**（任务 goroutine defer cli.Close()——终态保留期只保 task 记录 + rollingBuffer，绝不保留空闲连接，否则 32 个已完结任务可各挂一条连接最长 1h）。32 任务 = 至多 32 条**运行中**连接。
- **资源上界（内存）**：每通道 1 MiB × 2 通道 × 32 任务 = **64 MiB/project** 缓冲上限（serve 模式随 project 数线性放大）；连接同上按 project 封顶。文档（agent-access/README）如实点明。
...
```go
type rollingBuffer struct { mu sync.Mutex; buf []byte; cap, total int64; changed chan struct{} /* cap 1 */ }
// Write: 追加，超 cap 丢头；total 累计全流字节数；写后对 changed 做非阻塞 send（唤醒长轮询等待者）
...
- **Snapshot 在锁内深拷贝返回**（append/copy 出新切片），绝不返回内部 buf 的子切片——否则逃逸切片会被后续 Write 的丢头挪动腐蚀，且腐蚀全程持锁、**-race 抓不到**（该腐蚀路径在实验与语言语义上均成立，属必须靠规范禁止而非测试捕捉的类别）。
...
每任务一个聚合信号 `changed chan struct{}`（cap 1，非阻塞 send = 涌入合并），**两处喂**：① rollingBuffer.Write 落笔后（新字节）；② 任务状态离开 running 时（终态转换）。`exec_output` 的 wait 实现为 `select { <-changed / <-timer(wait 钳定值) / <-ctx.Done }`——收到信号后重算双通道快照决定返回。**禁止轮询 sleep 退化实现**（wait 小值测试 + `-race` 钉住）。唤醒原语归任务级而非缓冲级：单信号同时覆盖「新字节」与「终态」两个返回条件，避免 exec_output 同时等两个源。
...
- **上限 32/project（map 大小即计数）+ 满员驱逐**：`exec_background` 遇满员时，若存在终态项则**自动驱逐最旧的终态项**（删表项即释放缓冲；其 end 审计行已落，驱逐是保留期记账、不产生新审计行——与 sweeper 过期删除同级），驱逐后放行新任务；**运行中任务永不被驱逐**。仅当 32 项全为 running 才报错，引导文案：「wait for a running task to finish or call exec_stop」（此态下引导真实有效）。终态项的驱逐顺序 = `finishedAt` 最旧优先。
- **exec_stop 立即返回**（异步取消，不阻塞等终态——MCP 调用不占连接等 sess.Close 解阻，且解阻时长无界）：置 `stopRequested` + cancel 后即返回 `{status:"stopping"→实际返回当前态}`。**终态经 exec_output 观察**。对已终态任务 = 幂等 ok（回其终态，不产生新审计行）。
...
- **sweeper（1min tick，`SweepExpired` 可直调供白盒测试，照 SweepIdle 先例）**：
  ...
  - 满员驱逐（§上）在 exec_background 调用点同步执行，不归 sweeper；
...
入参：`task_id, wait_seconds?, stdout_offset?, stderr_offset?`。**offset 为对应通道的流内绝对字节偏移，`>=0`（schema 层拒负值）**；`0`/缺省 = 流首——首轮即落后会触发诚实降级，不静默跳。出参：
...
- **长轮询语义**：wait>0 时按 §2.3 的 changed 信号等到「任一通道有新字节（相对所传 offset）或任务离开 running 或 wait 钳定上限」才返回；tool-call ctx 取消 → 立即返回当前快照（不报错）。
...
入参：`task_id`。出参：`{ "status": "当前态" }`（运行中 → 触发停止后返回；终态 → 幂等回其终态）。**立即返回不阻塞**（§3）。运行中 → `stopRequested` 标志 + cancel → watchdog `Signal(SIGKILL)+sess.Close()`（**x/crypto/ssh 无信号楼梯可用：OpenSSH 服务端忽略 signal 请求，实际靠会话关闭 → 远端 SIGHUP；nohup/setsid 的远端进程会活——如实写进 agent-tools.md，与真 ssh 杀会话同语义**）→ 终态 stopped。未知 → 报错。
...
| 层 | 内容 |
|---|---|
| 单测：rollingBuffer | 游标推进 / gap（offset<start 的 lost 计数 + **next=total**）/ since≥total（空 chunk + **next=total 回拉**）/ 超 cap 丢头 / **并发读写 -race** / cap=0 边界 / **Snapshot 返回深拷贝**（返回后继续 Write，断言已取快照不被腐蚀——锁内别名腐蚀的显式回归） |
| 单测：runSession 抽取 | **前台回归网：exec/sudo 既有测试全绿零改动**（拆移不改行为的直接证据）；writers 直写断言 |
| 单测：TaskManager | 32 上限 + **满员驱逐**（最旧终态被驱逐、running 永不驱逐、全 running 才报错且引导文案真实）/ clampBackgroundTimeout、clampWaitSeconds 纯函数全分支 / sweeper 删终态过期项、不删 running / **终态即关 Client**（捕获 ref 断言连接已关而记录仍在）/ CloseAll 后表空 + client 全关 / stop 已终态幂等 / **stop 与自然退出竞态**（并发触发两路径，断言终态唯一、审计行恰一行、无锁外 check-then-act——同包用例 + -race） |
| 单测：testsshd 生命周期 e2e | 起后台（脚本逐行 sleep 输出）→ exec_output 拿增量（offset 推进）→ 自然退出拿 exit_code → **终态后取尾输出（offset 推到 next==total 无新增）——该断言同时钉住「Run 返回即 writer 排空」库不变量（实验已证，此处防库升级回归）** → stop 路径（cancel→stopped）→ timeout 路径（timeout_seconds=1 跑 sleep）→ sudo 路径；「exec_background 返回后连接仍活」断言；**failed 路径用 RST 代理 fixture 真触发（E2 实验装置产品化）** |
| 单测：ForProfile 包装 | profile 拒绝（denied）/ 全错误分支 **no-leak 断言扩三新工具**（Plan 31 assertBranch 分支签名锁同型）/ **failed 分支 no-leak（RST fixture，断言 error 文本过清洗）** / 审计行两型（start 全分支 + end 四终态，task_id 关联键）/ exec_output 无审计行断言 / 未知 task_id 报错文本含重启提示 |
...
| 单测：revoke 钉住 | `TestRevokedProjectKeepsBackgroundTaskRunning`（serve 路径 revoke 后任务不被杀，照隧道先例——若未来有人给 revoke 加 CloseAll 连带会红） |
...
| env seam | 三 env 生效 / **非法值与非正数均拒绝启动**（含 "0"/负值——ParseDuration 合法解析但语义破坏）/ 缺省回落生产默认值 |
