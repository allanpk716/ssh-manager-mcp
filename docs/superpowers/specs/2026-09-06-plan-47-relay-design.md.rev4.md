...
> 本版为第五版(四轮盲评收敛:r1 14 簇 / r2 13 簇 / r3 14 簇 / r4 kimi 7+codex 7 去重 11 簇)。**收敛信号:kimi r4 复核确认"r3 的 13 项修法逐条正确且自洽"、零高危;codex r4 三高全为 rev3 重构自身接缝,架构面四轮零发现。r4 要点**:①**状态表"真名在场"行按 chunk 覆盖度分流**(codex#1)——`manifest+partial+真名` 全在场是最终 rename 失败的合法产物(Windows 目标旧文件被占用),全块完成+partial 校验过 = **重试提交态**(missing=0 直奔 rename,不被逼 50GB 重传);`chunks=[]+source_size>0+真名在` = 覆盖旧真名的首跑中断自愈态(仅 size==0 才可能是提交碎片);②**fresh 空间预检计入可回收字节**(codex#2)——projected = Bavail + Stat(partial)(不解析 manifest),stage 0 删除后、数据移动前终检,消"盘满 fresh 永远到不了删除"死锁;③**砍"防御性复核"探针 rename**(codex#3)——续传路径试探 rename 会先覆写原 manifest,与"绝不写"矛盾;HasExtension 是唯一闸,首跑空清单落盘即真探针,续传侧广告但不支持最坏浪费 ≤1 块(登记接受);④**恢复 endpoint 限定键**(kimi#1/codex#4)——write 键=`to_server_id+"\x00"+canonical 工件路径`、read 键=`from_server_id+"\x00"+canonical 源路径`(rev3 重写时丢失——同文件分发多台服务器=本 plan 分发债核心场景会被误串行);⑤admission 测试半边补齐+计数回滚断言(kimi#2)、"32"全部改"maxTasks(默认 32,SSHMGR_BG_MAX_TASKS 可调)"(kimi#2/codex#6);⑥审计 start 词表补"运行中冲突、任务表满员"(kimi#3);⑦§1.2 判据措辞同步"byte 0→EOF 单程(含空清单自愈)报 file_sha256"(kimi#4/codex#5);⑧§5 写放大数字修正(16384 块累计 ≈17 GB;4TB 级 ≈0.4%;16MiB 配满 ≈7%——kimi#5);⑨**StatVFS 逐级上溯最近存在祖先**(kimi#7——MkdirAll 移引擎后首跑新目录预检恒退化 unavailable 的功能性回退,修法便宜);⑩本机源 e2e 补全(kimi#6——第二大卖点零 e2e);⑪溢出安全 ceil(q/r 形态、size<0 拒)+ manifest 校验补 `0≤i<n`、chunks 数组存在性(codex#7)。评审留底:.xcheck/plan47-r{1,2,3,4}-{kimi,codex}.out。
> **r3 要点(前两轮修法的残留接缝,历史)**:①**fresh 语义重构**——⑥ fresh 分支不解析 manifest 语义、不删除(先解析后删除使 fresh 无法修复 malformed manifest——codex#5),删除移入引擎 stage 0 且以 ⑧ 租约为前提(rev2 序会让并发 fresh **先删掉运行中任务的 50GB 进度**再被闸拒——kimi#1/codex#1);②**posix-rename 探测解耦**——stage 0 无条件 `HasExtension("posix-rename@openssh.com")` 零 IO 查询先行(探针 rename 绑在空 manifest 落盘上,续传路径零探测、最晚到提交点才爆——kimi#2/codex#4;区分"不支持"与 IO 错误);③**ReserveRelay = Reserve 超集原语**——admission(32 上限/驱逐/ErrBgTaskLimit)+工件/读写集查重+锁内 insertLocked 一体(Insert 自取 m.mu 不可重入,字面"同锁转正"自死锁;占位仅在锁闭区间内存在、对外不可见,不被驱逐/清扫误删——kimi#3);④**read/write 冲突集**——源路径可以是同目标 partial/manifest(fresh 删自己的源、非 fresh truncate 正在读的 partial——codex#2):参数层 own source ∉ own 工件集;闸 = write∩(running writes∪reads)=∅ 且 own read∩running writes=∅,read-read 允许;⑤**canonicalize 一次贯通**——参数层 path.Clean(远程)/filepath.Clean(本机)后同一值用于同径判定/Stat/MkdirAll/工件构造/元数据/审计(`/a/./b` 绕过 + 尾斜杠工件漂移——kimi#7/codex#3);⑥MkdirAll 直调 `sc.MkdirAll(path.Dir(…))`(WriteFile 同款,不经 Client.MkdirAll 的 ToSlash——kimi#8);⑦块读钉 `io.CopyN(dst, src, min(chunk, size−offset))`(字面 io.Copy 非末块会把源剩余全灌进 partial——kimi#5);⑧空间余量改 `min(chunk, missing)`、missing=0 免余量(zero-byte 与只差提交的续传被过度拒——kimi#6);⑨审计词汇表 start/end 两行分列(补 auth_error、no_credential 双端、end 行 ok/stopped/timeout/failed——kimi#4/codex#9);⑩file_sha256 判据=「本任务从 byte 0 连续流到 EOF」而非 fresh 参数(自愈态两可——kimi#9);⑪env 解析提前到 manager/sweeper 构造**之前**(非法值返回 error 时已启 goroutine 不收口——codex#6);⑫**relayMaxChunks=16384 常量闸**——preflight fail-closed,挡 manifest O(n²) 写放大在 16MiB 块×TB 级失守(codex#7;TB 级 journal 化结构登记 backlog);⑬状态表"manifest 在+partial 无+真名无"按 chunks 空否分流(chunks≠[] 是非常态,不能泛化成续传——codex#8)。评审留底:.xcheck/plan47-r{1,2,3}-{kimi,codex}.out。
...
- **运行中冲突拒绝(read/write 冲突集;rev4 恢复 endpoint 限定键——kimi#1/codex#4:rev3 重写时把 rev2 的键构成公式丢了,字面实现会把"同一文件分发到多台服务器"——本 plan 分发债核心场景——误判冲突全部串行)**:经 **ReserveRelay 持锁原语**(§2⑧,= 既有 Reserve 的**超集**:admission maxTasks 上限+满员驱逐+工件/读写集查重+锁内 insertLocked 一体)。每个 relay 登记 **write-set**(四工件)+ **read-set**(仅远程源;本机源不登记——broker 本机文件不在远端冲突域)。**键构成(rev4 钉死)**:write 键 = `to_server_id + "\x00" + canonical 工件路径`,read 键 = `from_server_id + "\x00" + canonical 源路径`——四个判定矩阵全部在此同构键空间求交,跨服务器同路径零互斥。判定:own read ∩ own write = ∅(参数层先拒,同 endpoint 下才成立——fresh 会删自己的源、非 fresh 会 truncate 正在读的 partial,codex#2);own write ∩ (任一 running 的 writes ∪ reads) = ∅;own read ∩ 任一 running writes = ∅;read-read 允许。占位**仅在锁闭区间内存在、对外不可见**(不进 status 枚举、不被驱逐/清扫误删)。**如实声明**:跨 project(serve.go scopedServer per-project)不在保护面内,兜底=B 端事实源 + 首块前空 Manifest;大小写/symlink 别名登记残余(§9)。
...
preflight(同步段,零网络字节移动、**零远端状态变更**——一切 mutation(MkdirAll/fresh 删除/空清单)移入引擎或闸后,rev3 kimi#1/codex#1/#5):
  ...
  ③b **块数闸**:size<0 → 拒;**溢出安全求块数(rev4 codex#7)**:n = size/chunk_bytes,
     size%chunk_bytes != 0 则 n++(禁 (size+chunk−1)/chunk 形态——size 近 MaxInt64 时先
     溢为负绕闸);n > relayMaxChunks(=16384 常量)→ refusal(错误文本带两值并指引
     "raise SSHMGR_TRANSFER_CHUNK");挡 manifest O(n²) 写放大与清单无界增长(16MiB 块×TB
     级)。16384 覆盖 4TB@256MiB;TB 级 journal 化清单结构登记 backlog(§9)
  ...
  ⑦ 空间预检(fresh/无 manifest 时 missing=source_size,否则 missing=source_size−已完成字节):
     口径 = **Available 单值**(Bavail)+ **fresh 投影回收(rev4 codex#2)**:fresh=true 时
     Stat(partial)(不解析 manifest)得其字节计入可回收——projected = Bavail + partialSize,
     消"盘满时 fresh 永远到不了 stage 0 删除"死锁;引擎 stage 0 删除后、任何数据移动前
     **终检 StatVFS**(不足 → failed,零块移动——盘已腾,残留清理责任归 fresh 语义)。
     需求 = missing + **min(chunk_bytes, missing)** 余量(rev3 kimi#6:zero-byte 与
     missing=0 的只差提交续传不再被整块余量过度拒);uint64→int64 安全转换。
     不足 → refusal(不建任务)。**StatVFS 定位路径 = 目标父目录,不存在时逐级上溯至
     最近存在祖先(rev4 kimi#7——MkdirAll 移引擎后首跑新目录的预检不恒退化
     unavailable;上溯到根仍失败 → unavailable)**;其余不支持/报错 →
  ...
  ⑧ **ReserveRelay——rev3 = Reserve 的超集原语(kimi#3/codex#1/#2)**:tm.mu 锁内一体完成
     ...
     文案——relay 不绕 maxTasks 上限(默认 32,`SSHMGR_BG_MAX_TASKS` 可调,rev4
     kimi#2/codex#6——"自然约束"字面落地)+ **read/write 冲突集查重**(§1.1
...
引擎 goroutine(runRelay——复用 runTask 骨架,exec 闭包泛化为引擎闭包,终态纪律零复制:
  ...
  stage 0(首块前;顺序钉死——rev3:**一切 mutation 以 ⑧ 租约为前提**):
    - **posix-rename 硬依赖探测(无条件先行,rev3 kimi#2/codex#4)**:`HasExtension(
      ...
      relay requires it for atomic manifest updates"(与 IO 错误分流)。**rev4 codex#3:
      无防御性探针 rename**——续传路径对 manifest 做试探 rename 会先覆写原清单(在抽读
      复核之前),与"绝不写"矛盾;首跑的空清单真实落盘即活探针,续传侧"广告但不支持"
      最坏浪费 ≤1 块后失败(登记接受,§9),提交点 rename 失败由 §3 重试提交态兜住。
      **Remove+Rename 回退矩阵整体砍除**(回退窗×200 块=崩溃即进度全毁,不做概率性
      原子;OpenSSH 全系含 Win32 端口支持该扩展,兼容面收窄登记 §7/§9)
    ...
    - **fresh 删除(rev3 从 ⑥ 移入,kimi#1/codex#1/#5)**:fresh=true 时删 partial + manifest
      ...
      的工件不可能被误删;**删除后、任何数据移动前终检 StatVFS(rev4 codex#2——预检是
      投影值,删后不足 → failed,零块移动)**
...
- **解析防御**:stat 大小 > `⌈source_size/chunk_bytes⌉×128B+8KiB`(推导上限,随块数伸缩)→ 拒;结构校验(version/chunk_bytes/**chunks 须为数组**[rev4 codex#7]/index 唯一且 **0 ≤ i < ⌈size/chunk⌉**/64-hex/完成字节 ≤ size)。
- **状态组合判别(唯一权威表,§2⑥ 引用此处;rev4:"真名在场"行按 chunk 覆盖度分流——codex#1:最终 PosixRename 失败[Windows 目标旧文件被占用]是合法产物,不可一律判 fresh 逼重传)**:
...
| 在(chunks≠[],未全完成) | 无 | 无 | **非常态**(手动删 partial 等)→ 拒,fresh 指引——不可泛化成续传(stage 0 重建 partial 会把不存在数据当作已完成) |
| 在(全完成) | 无 | 无 | 中断于提交前 → 续传=直奔提交(missing=0) |
| 在 | 有 | 无 | 中断 → 续传(partial 过结构校验+尺寸收敛;全完成=直奔提交) |
| 在(全完成) | 无 | 有 | **提交成功碎片**:拒,明示无害+清理指引,不逼重传 |
| 在(chunks=[], size==0) | 无 | 有 | 提交成功碎片(零字节传输的 debris 形态)同上 |
| 在(chunks=[], size>0) | 无 | 有 | **覆盖旧真名的首跑中断自愈态**:续走,完成时替换真名 |
| 在(未全完成) | 无 | 有 | 非常态 → 拒,fresh 指引 |
| 在(全完成,partial 校验过) | 有 | 有 | **重试提交态**(最终 rename 失败的合法产物)→ 续传=重试提交,不被逼重传 |
| 在(未全完成) | 有 | 有 | 非常态 → 拒,fresh 指引 |
...
- **远端写放大(rev4 数字修正,kimi#5)**:manifest 每块全量重写 = O(块数²) 累计写——50GB@256MiB=200 块 ≈ 2.4 MB 累计,可忽略;**relayMaxChunks=16384 把最坏情形钉在界内**:满块数累计 ≈ 128B×N(N+1)/2 ≈ **17 GB**——4TB 级(256MiB 块)相对数据 ≈**0.4%**,16MiB 最小块配满块数(256GB 数据)时 ≈**7%**(仍为有界、由块数闸钉死;RTT 开销才是主成本且可忽略);1 次 rename + 1 次 manifest 删除。TB 级 journal 化清单(append-only,免全量重写)登记 backlog(§9)。
...
- **statuses(rev3 两行分列,kimi#4/codex#9)**:
  - **start 行(preflight,含 ③④ 连接分支)**:`ok / denied / auth_error(vault 非 ErrNoCredential 失败)/ no_credential(**双端**——dest ④ 同走 AuthForServer)/ hostkey_mismatch / connect_error / cancelled / error`(空间不足、manifest 结构非法/不符、状态组合非常态、目标路径形态错误、块数闸、参数非法、**运行中读写集冲突、任务表满员**[rev4 kimi#3 补]均归 error)。
...
- **单测(mcpserver / RelayForProfile,core_test.go 同层)**:
  ...
  - 空间分支:不足 → refusal(avail/need 文本;续传按 missing bytes 口径——预置大 partial 不再误拒);**fresh 投影:StatVFS 不足但计入 partial 可回收字节后足够 → 任务照建(rev4 codex#2 锚)**;StatVFS 报错 → unavailable 且任务照建;**父目录不存在时逐级上溯祖先后预检恢复有效、上溯到根仍失败 → unavailable(rev4 kimi#7 锚)**。
  - Manifest §3 状态表**逐行**用例(14 行各一,含真名在场覆盖度分流——rev4 codex#1 锚);结构非法拒(越界/重复 index/非 64-hex/**负 index、chunks 非数组——rev4 codex#7**)/ 推导上限拒(小 chunk×大 size 模拟超限清单数;**MaxInt64 级 size 的溢出安全块数求值——rev4**);chunk_bytes/size/mtime 不符拒;fresh=true 清零断言;**malformed/超限 manifest + fresh=true 可重启(⑥ 对 fresh 跳过解析——rev3 codex#5 锚)**。
  ...
  - ReserveRelay:同工件集**真并发**启动只成一败一;**不同 to_server_id 同路径并行不互斥(rev4 kimi#1 锚——分发场景反例)**;**同 server 工件碰撞拒**;`/tmp/a\b` 与 `/tmp/a/b` 不同键;**read∩write 冲突拒(源=自身工件[参数层]/源=running 任务的目标工件[闸,同 endpoint]——codex#2 锚)**;**`/a/./b`、`/a//b` canonical 后同键不可绕 ①(codex#3 锚)**;**fresh=true 并发重发同目标:运行中任务的远端工件零触碰(删除在租约后——kimi#1/codex#1 锚)**;**admission 半边(rev4 kimi#2 锚):自定义 SSHMGR_BG_MAX_TASKS 下满员驱逐最旧终态后占位成功 / 无终态可逐 → ErrBgTaskLimit 引导文案 / ⑧ 占位后一切失败路径 `reserved` 计数归零回滚断言(计数漂移会让任务表远低于上限即拒一切新任务,含 exec_background)**。
...
- **e2e(e2e_test.go)**:工具集合等式 11→12;全流程 + sha256sum 对 file_sha256;续传全流程(Stop→重跑→只补缺块,源读字节计数断言→字节精确+rename+manifest 删);**首块前中断:双件齐可续传**;**清单后 partial 前中断:自愈态直走**;**最终 rename 失败注入(占住真名)→ 重跑走重试提交态,零块移动完成(rev4 codex#1 锚)**;zero-byte 端到端;**本机源全流程 + 中断续传(rev4 kimi#6 锚——第二大卖点:testsshd 目标,断言字节精确+rename+manifest 删;本机源文件不被 fresh 触碰)**;**manifest 删除失败注入 → done+警告+真名在**;broker restart 语义模拟(任务表清空重跑续传)。
...
- **断线自动重连**(重跑=恢复)。
- **relay 并发任务数专用 cap**(maxTasks 自然约束——默认 32,`SSHMGR_BG_MAX_TASKS` 可调)。
- **续传侧"广告但不支持"posix-rename 的 ≤1 块浪费(rev4 登记)**:HasExtension 报支持但实操失败的目标,续传任务最坏传完一个缺块后在 manifest 更新处失败——≤256 MiB 重传单位,接受。
