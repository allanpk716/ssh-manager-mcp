...
- 日期:2026-08-29
- 状态:rev1(第 1 轮盲评 25 条发现全闭环:24 采纳 + C1 owner 拍板"维持现状+诚实登记";待第 2 轮盲评)
...
| Q2 | 信任链 | 强制 https(仅环回字面量例外)+ 重定向**每一跳**宿主白名单(4 宿主)+ 同 release checksums.txt SHA256;不做签名;`SSHMGR_UPDATE_BASE` env seam;零新依赖 |
| Q3 | 替换与重启 | Windows rename-to-`.old` / 其余原子 rename;staged 自检;崩溃窗口自愈;文件替换免提权(NUC10 用户目录),服务重启经 kardianos `Restart()`,失败打印手工命令;重启前校验服务注册路径;client 零中断 |
...
**旧服务检测**:update **无条件**探测新旧两名(kardianos `service.New`→`Status()`);任一探测返回非 `ErrNotInstalled` 的错误 → **fail-closed 中止**;旧名存在(无论新名状态)→ 打印上述迁移块并中止(不半更新,防新旧服务并存);`ErrNoServiceSystemDetected`(容器/CI 无服务管理器)→ 跳过检测直接更新。
...
- 解析 `tag_name` + `assets[].name/browser_download_url`;目标资产名本地计算:`sshmgr_{ver}_{GOOS}_{GOARCH}.{zip|tar.gz}`,ver=tag 去掉 `v` 前缀,windows→zip 其余→tar.gz;本地 GOOS/GOARCH 不在发布矩阵(windows/linux/darwin × amd64/arm64)→ 明确报错并列出支持矩阵
- 版本比较:手写三段数字比较(`vMAJOR.MINOR.PATCH[-pre]`,pre 使同号偏旧)——不引 semver 依赖;`buildinfo` 增 `Owner="allanpk716"`/`Repo="ssh-manager-mcp"` 常量;**当前版本解析失败(含本地构建默认 `dev`)→ 拒绝自动比较,报错要求 `--version` 显式指定**
- 未认证 API 限速 60/h/IP——手动更新场景足够,文档注明
- 超时:API 30s;下载流式落盘,**idle 60s 零字节或总超时 10 分钟即中止**,上限 200MiB 防异常资产

### 4.2 下载与校验

1. 下载 checksums.txt 资产 → 解析(多行 `sha256  文件名`,容 CRLF)→ 取目标资产行(缺行拒绝)
2. 下载目标资产 → SHA256 全量比对,不符即中止(**目标文件零触碰**)
3. **解压策略(钉死,zip slip 不可能)**:流式遍历归档,**只落地名称精确等于 `sshmgr`(windows 为 `sshmgr.exe`)的根条目**,其余条目一律不写盘;拒绝绝对路径/`..`/子目录/重复名/symlink/hardlink/device/非 regular 文件;单成员与总解压字节上限;以 `O_EXCL` 创建写入临时目录
4. 传输安全:**https 强制,唯一例外=`SSHMGR_UPDATE_BASE` 指向环回字面量(127.0.0.1/::1/localhost)时允许 http(测试 seam)**;`http.Client` 的 `CheckRedirect` 对**初始 URL 与每一跳**校验 scheme+exact host(不只最终宿主):默认白名单 `{api.github.com, github.com, objects.githubusercontent.com, release-assets.githubusercontent.com}`(**实测本仓资产 302 宿主为 release-assets**;GitHub 若再增 `*.githubusercontent.com` 子域属维护点);**`SSHMGR_UPDATE_BASE` 显式设定时**:白名单换为 base 宿主(仍强制 https,环回除外)
...
校验通过后:
  staged 自检:执行 staged 二进制 `version`,输出规范化后 == 目标 tag 才继续
    (防 checksum 正确但架构错/坏包的资产打断服务;失败→保留原文件,清 staged)
  GOOS=windows:
    清残留 self+".old"(尽力,失败仅警告)
    os.Rename(self, self+".old")            # 运行中 exe 可改名不可删
    os.Rename(tmp/self, self) 失败 → 回滚 Rename(.old→self)
      回滚再失败 → 专用退出码 + 打印手工恢复命令(ren sshmgr.exe.old sshmgr.exe)+ 跳过重启询问
  其余(Linux/darwin):
    chmod 0755 → os.Rename(tmp/self, self)   # 原子,运行中进程持旧 inode
  新文件 + 目录 fsync 后才算替换成功
启动自愈:update 起手检测 self 缺失但 self+".old" 存在 → 交互确认后恢复(两 rename 之间崩溃的窗口)
```

- 任一步失败,目标路径字节不变(校验/staged 失败在 tmpdir 内中止;写入失败回滚 rename)——**不变式:失败=零变更**
...
- 新名服务 `sshmgr-serve` 已注册:替换成功后询问"重启服务立即生效?[Y/n]"(`--yes` 跳过询问=同意);**重启前校验服务注册二进制路径 == self**(Windows 读 SCM binPath / Linux 读 unit ExecStart;读不到→醒目警告继续),不一致 → **中止并打印两端路径**(防"更新了 A 路径 exe、服务跑 B 路径"静默旧版);执行 `s.Restart()`
- `Restart()` 失败(典型:非提升会话操作 SCM/systemctl)→ 打印平台手工命令,非零退出:
...
- **非 TTY(stdin 非 tty)且无 `--yes` → 报错退出要求 `--yes`**(一切确认类交互同此规则;G10 MCP 代跑场景即命中)
- **降级**(`--version` 目标 < 当前):即使 `--yes` 也打印醒目行"降级至 vX(回滚通道)";无 `--yes` 时确认文案明示这是降级
...
- 输出证据行:版本对、资产名、SHA256 命中、**update base(非默认时额外醒目)**、替换路径、重启结果——供真机验收留档与真假源区分

### 4.6 env seam(生产路径必须留缝,SSHMGR_CACHE_DEK 教训)

- `SSHMGR_UPDATE_BASE`:默认 `https://api.github.com`;显式设定时白名单换为 base 宿主;**仅环回字面量允许 http**(测试假源),其余强制 https——一条规则消除"生产镜像 vs 测试 http"的矛盾
- 代码内可注入 seam:版本发现/下载经 package 级函数变量(单测 httptest 替身);服务检测经 `service.New` 函数变量(测旧名/双服务/无管理器分支)
...
- **单测**(零新依赖,httptest+注入 seam):
  - 版本比较三段式表测(含 pre 后缀、非法输入拒绝、**`dev` 拒绝自动比较**)
  - 资产名计算(GOOS/GOARCH 矩阵、v 前缀剥离;**矩阵外平台报错分支**)
  - checksums.txt 解析(多行/CRLF/缺行拒绝)
  - **解压安全矩阵**:`../evil`、绝对路径、子目录同名条目、symlink/hardlink、重复名、非 regular——一律拒绝且零落地;正常黄金夹具(zip/tar.gz)通过
  - **事务性**(临时副本+rename 失败注入,单测可跑,不限平台):校验失败→目标字节不变;写入失败→回滚→目标字节不变;**staged version 自检失败(假输出版本)→拒绝**;**回滚 double-fault→专用退出码+恢复指引文案**;**启动自愈分支**(self 缺失+.old 存在→恢复)
  - 传输:非环回 http 拒绝;**环回 http 放行**;**每一跳白名单**(经 redirect 链造非法中间跳→拒);`SSHMGR_UPDATE_BASE` 自定义宿主放行+越界宿主拒
  - 服务检测(seam 替身):旧名存在→迁移块+中止;**双服务并存→中止**;**Status 错误→fail-closed 中止**;无服务管理器→跳过;**注册路径≠self→中止打印两端**
  - **非 TTY 且无 --yes → 报错退出**(注入 stdin 状态)
...
| G10 | update 回路:NUC10 上 `SSHMGR_UPDATE_BASE` 指环回假源(构造 v0.13.0→v0.13.1-test)`sshmgr update --yes`(非 TTY 规则命中)全链,断言替换+服务重启+版本翻转+证据行含 base,再指回真源自愈 | 我经 MCP 代跑 |
...
| G12 | 旧服务名场景矩阵:旧名注册→迁移块+中止;**双服务并存→中止**;**Status 查询失败→fail-closed**(构造) | 我经 MCP 代跑 |
...
- `threat-model.md`:R13 自更新供应链(含 §5 子风险四条)+ §6 更新面收窄声明
