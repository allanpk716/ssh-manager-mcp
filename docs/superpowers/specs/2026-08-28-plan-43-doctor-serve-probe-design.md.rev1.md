# Plan 43 — doctor serve 探活二期（绿/黄/红语义）设计 spec · rev1

- 日期：2026-08-28（rev1 同日：异构盲评 2 家 13 条反馈闭环——11 条必改全吸收，1 条修法 owner 拍板降 WARN；实证补强见 F9/F10）
...
- 状态：rev1 定稿待 owner 审（rev0 = 本文件去后缀原稿；修订点见 §9 变更记录）
...
- **F9（实验 rev1，真 ServeRunner + 真临时 store + 真码，2026-08-28）**：
  - HEAD(活码) → **405, bodyLen=0**；HEAD(垃圾码) → **401**——F3 阶梯在**真实 handler** 上坐实（非模拟）。
  - GET(活码) → **200 且 `LastPullAt` 从零值被推进**——touch 副作用实锤（F2 的成功路径面）。
  - revoke 后同 name 重发 → **新 token ≠ 旧 token**；同 bearer 串的 invalid→valid 迁移**经 store API 不可构造**（revocation 终态 + GenerateToken 随机铸码）——§3.1-4 的竞态分支**构造不出来**（带外手工改库除外）。
- **F10（实验 rev1，2026-08-28）**：敌意 401 body「access revoked by upstream policy」——裸子串 `revoked` 匹配**误标 revoked**；加 `invalid cache token:` 前缀闸后正确归 unknown-code。真 serve 两种 body 两规则判定一致。→ reason 判定必须带前缀闸。
...
const (
    ...
    ProbeBadStatus   ProbeClass = "bad-status"    // 其他 HTTP 状态（含 3xx——恒不跟）；本地拒绝（无 pin/非 https）
...
2. **本地闸（零网络短路，rev1 修改）**：`pin == ""` **或** `cred.URL` 非 `https://` scheme → **不发任何请求**，直接返回 `ProbeBadStatus` + 固定文案——pin 空用「credential has no pin (plaintext-grade relic) — refusing to send the device code over an unpinned channel; re-pull with a pinned code」；非 https 用「credential URL is not https — refusing to send the device code in cleartext; re-pull with an https URL」。设备码**只在 pin 保护的信道上发送**（与 pull 的明文拒绝同反射）。
...
5. 响应阶梯：
   ...
   - `401` → **补一发 GET**（同 transport 同 URL 同码）→ 读 body ≤8KiB（pull 同款上限）→ **先匹配 `invalid cache token:` 前缀**，命中后再取 reason 词：含 `revoked` → **ProbeRevoked**，否则 **ProbeUnknownCode**（前缀不命中 → 一律 ProbeUnknownCode，防前置代理敌意 body 误标，F10）。reason 词或 body 截断 ~60 字节进 Detail。GET 若 `200`（HEAD↔GET 间码被换活——**经 store API 不可构造**，F9-D；仅带外手工改库可造）→ ProbeActive + Detail 注 `(race: code re-activated between HEAD and GET)`；GET 传输错 → 按第 6 步分类。
   - 其他状态（3xx/5xx/…，3xx 因不跟而停在此）→ **ProbeBadStatus**（状态码进 Detail）。
6. `client.Do` 错误分类：错误串含 `server fingerprint mismatch`（pinningTransport 的 VerifyConnection 文案，F5）→ **ProbePinMismatch**（Detail 只报 mismatch 事实，**不回显期望/实际指纹**——指纹虽公开，探针 Detail 保持最小面）；其他传输错（超时/拒绝/DNS/非指纹 TLS 错，含证书过期）→ **ProbeUnreachable**（错误类短词进 Detail，如 `timeout` / `connection refused`，不透传原始串——防错误文本携带内部 URL 细节）。**残余**（NewRequest 失败、GET 补发的 body 读 io 错等无法归类者）→ **ProbeInternal**。
7. **零副作用承诺（rev1 精化）**：探针只读——永不 Quarantine、永不写任何文件；服务端唯一效应 = 401 时的 stderr 日志行 + 前缀回查（既有可观测性面，F4）。**已登记例外**：若带外手工改库造出 HEAD-401→GET-200 竞态（F9-D：经 API 不可构造），单次 touch + 单次快照序列化发生——有界、已登记、不视为承诺破坏。
...
- **seam**（`serveServiceState` 先例，doctor 测试零真网络）：
  ...
  var probeTimeout  = 3 * time.Second         // 仅 client broker 探针（rev1 措辞修正：server 侧 1s 是 probeServeHTTP 自带）
  ```
- **角色路由**（一个 check 函数输出全部行）：
  - 无 role → 一行 INFO `serve-probe`：「no role — nothing to probe」。
  - `server` → 若 serve-svc 状态 NOT INSTALLED → INFO「serve not in use — nothing to probe」；否则 `probeLoopback("127.0.0.1:7878")` → 行 `serve-probe`：true = PASS「serve responding over TLS (401 = auth gate up)」（无 elapsed——`probeServeHTTP` 只回 bool）；false = **WARN（rev1 修改，owner 拍板）**「serve not responding on 127.0.0.1:7878 (default addr; custom --addr installs or broker down — verify with `ssh-manager serve status`)」——service Running + loopback 不可达是歧义态（custom-addr 装机健康机不可误红 exit 1；真崩溃由 serve-svc 行的 Stopped WARN 承担红旗）。**server 侧探针无 FAIL 分支**。
...
| BadStatus | 🟡 WARN | `instance <n>: broker answered <status> (expected 405) <—或本地拒绝固定文案>` | `inspect serve on the broker machine (serve status / serve.log)` |

（server 角色三态已在路由给出：PASS / **WARN**（rev1）/ INFO。Internal 不在本表——它不是被诊对象的发现，不产 doctor 行，见 §3.3。）

### 3.3 exit 2 边界、优先级与 Detail 脱敏（Plan 38 预埋验收①；rev1 补脱敏冻结）
...
- **internal Detail 脱敏（rev1 冻结）**：internal 错误**永不透传原始 err 串**（URL parse/NewRequest 错误常内嵌原始 URL，userinfo 形态可泄）——固定安全文案 + 错误类短词（如 `request construction failed` / `response body read failed`）。与 §3.1-6 传输错误的「不透传原始串」同一纪律。

### 3.4 帮助文本（Plan 38 预埋验收②，冻结文案；rev1 措辞修正）
...
（rev1：去掉 `(probe machinery)` 括号——exit 2 语义是通用 doctor 内部错误（Plan 38），本 plan 只是第一个产源；probe 的提及留在 Long 段。）

### 3.5 Detail 信息纪律

- 探针 Detail **绝不含**：码值（任何形态/前缀）、URL 的凭据段、pin 指纹值（含 mismatch 时的期望/实际）、**internal 错误的原始 err 串**（rev1）。允许：状态码、reason 词、错误类短词、elapsed、实例名、age/cap。
...
8. **本地闸（rev1）**：pin 空与 URL 非 https 两腿 → ProbeBadStatus + 固定文案 + **零网络断言**（stub/httptest 计数为零）。
9. **reason 前缀闸（rev1）**：GET 401 敌意 body「access revoked by upstream policy」→ ProbeUnknownCode（非 revoked，F10）。
10. **零副作用钉子**：探针前后 cache 目录快照逐字节不变（无 quarantine/ 产物、无文件 mtime 变化）。
11. **真实 internal 触发腿（rev1）**：cred.URL 含过本地闸的非法控制字符（如 URL 带空格经 url.Parse 容忍、`http.NewRequest` 拒绝的形态）→ ProbeInternal + 固定安全文案断言（不含原始串）。
12. Detail 纪律：全表断言不含码值/pin 串/原始 err 串。

### 4.1b 真 handler 契约测试（rev1 新增，mcpserver 包——`serve_test.go` 既有真 ServeRunner 姿势）
...
5. server 分支：NOT INSTALLED→INFO / loopback true→PASS / **false→WARN（rev1：歧义态文案含 custom-addr 提示）**。
6. 帮助文本断言：`--help` 输出含 exit 2 行（无 probe 括号）+ no-network-by-default 句。
...
- **不做** `--probe-addr`（server 探默认 addr；custom-addr 装机的歧义以 WARN + 指引表达，rev1 拍板）。
- **不做**并行探针/结果缓存（顺序逐实例，**per-instance 最坏 2×3s=6s**（死码 HEAD+补发 GET 两发各吃满 timeout，rev1 修正），N 实例总上界 N×6s——诊断场景可接受）。
...
- threat-model：`--probe` 发送设备码（**仅 pin 保护信道上**——无 pin/非 https 本地短路拒绝，rev1 强化；与 pull 同信道同险）；doctor 输出永不回显码值——一句话登记。

## 7. 验收清单

1. **Plan 38 预埋①**：internal 经 `NewExitCodeError(2, ...)`（4.2-2 测试钉）。
2. **Plan 38 预埋②**：帮助文本 exit 2 行上线（4.2-6 断言）。
3. **默认契约**：无 `--probe` 的 doctor **默认诊断报告输出**与本改动前逐字节等价（rev1 限定范围：新增 flag/帮助文本变更不计入——`--help` 输出按 §3.4 变更）。
4. clientops 探针全表绿（4.1 十二项）+ 真 handler 契约测试绿（4.1b）。
5. 全仓测试绿。
6. 真机（发版后双端）：NUC10 `--probe` = `serve-probe PASS`（loopback）；笔记本 `--probe` = `serve-probe PASS`（pinned 405 活码）。owner gate。

## 8. 风险与备选

- **R1（rev1 加强）**：阶梯依赖 `handleSnapshot` method-check-after-auth 代码序——**已被 4.1b 真 handler 契约测试钉住**（F9 实验先行验证）；serve 未来重排此序即红（fail-loud），且 405|200 双收已吸收「允许 HEAD」的未来演进。
...
- **R5（rev1 新登记）**：server 侧探针无 FAIL 分支（owner 拍板 WARN 化）——「broker 机真崩 + service 状态误报 Running」的复合态下 doctor 总判 WARN 非 FAIL；部署验证脚本若依赖 exit 1 捕获 server 崩溃，需改看 serve-svc 行或 serve status。接受面：serve-svc Stopped 已 WARN、真崩通常伴随 Stopped/NOT INSTALLED。

## 9. rev1 变更记录（相对 rev0）

1. §3.1-2：pin 空 / 非 https → **本地短路零网络**（原措辞隐含无 pin 发码——安全级修复）。
2. §3.1-5：reason 判定加 `invalid cache token:` 前缀闸（F10 敌意 body 误标）。
3. §3.1-7：零副作用承诺精化——竞态例外有界登记；竞态描述升级「经 store API 不可构造」（F9-D）。
4. §3.2：server loopback 不可达 FAIL→**WARN**（owner 拍板；custom-addr 歧义态不可误红）。
5. §3.3：internal Detail 脱敏冻结（固定文案+错误类，不透传原始 err 串）。
6. §3.4：exit 2 行去 `(probe machinery)` 括号（通用语义）。
7. §4.1：+本地闸两腿（8）、+前缀闸腿（9）、+真实 internal 触发腿（11）。
8. §4.1b：**新增强**——真 handler 契约测试（F9 断言集）。
9. §5：per-instance 上界 3s→**2×3s**；custom-addr 以 WARN+指引表达。
10. §7-3：等价范围限定为默认诊断报告。
11. §2：+F9/F10 实验事实；§8：R1 加强、+R5 登记。
