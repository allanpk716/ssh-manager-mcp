...
- 日期：2026-08-20
- 状态：rev1。已与 owner 对齐方向（MCP 工具手册口径、三交付物方案 A）；本版按一轮外部反馈的逐条查证与实验结论修订（关键实验结论：serve 端点对 URL 尾斜杠/子路径**功能无关**；拼接式 JSON 渲染对含引号/反斜杠的 URL **不健壮**）
...
| **实验事实（2026-08-20，拼接渲染 + json 校验）**：按拼接法渲染，含 `"` 或 `\` 的 URL 产出**非法 JSON**（`invalid character …`）；常规/尾斜杠 URL 合法。**拼接式渲染必须配值转义** | 实验留底：jsonbreak 探测器输出 |
...
- **snippet 归属（钉死）**：`snippet` = `mcpConfigLines` 的**完整输出**——含首行引导语「把下面的片段写进 agent 项目的 .mcp.json：」、JSON 块、以及末尾「说明：」注记块；`snippet == nil` 时**该整块不渲染**（这就是零值兼容的成立条件：设备码/向导发射点不填 snippet → 输出即现状裸 token）。
...
body 结构（渲染后形态；snippet 块 = `mcpConfigLines` 完整输出，token 真值代入）：
...
- 联机场景另有两种形态：在线 http / 离线 --cache，配置示例见 docs/multi-machine.md「离线只读缓存」Step 2。
...
- **取舍说明（有意设计）**：secretView 片段统一以 stdio 形态为主形态——主控台是熟练操作场景、与 CLI `printToken` 输出对齐；联机受众的完整双形态引导由 `clientFinishScreen`（G2）与文档承担，此处仅一行指针带出。
...
// VALUE ESCAPING (hard requirement): urlRef / tokenRef are interpolated
// through strconv.Quote-derived escaping (or encoding/json value encoding)
// before landing in the JSON lines — raw concatenation produces INVALID
// JSON for URLs containing '"' or '\' (verified experimentally 2026-08-20).
func mcpHttpConfigLines(urlRef, tokenRef string, notes []string) []string
```

渲染形态（结构对齐 multi-machine.md「离线只读缓存」Step 2 的在线示例；值经转义）：
...
```
{
  "mcpServers": {
    "ssh": {
      "type": "http",
      "url": "<urlRef（转义后）>",
      "headers": { "Authorization": "Bearer <tokenRef（转义后）>" }
    }
  }
}
```
...
finish 屏结构：标题不变（「配置 agent 的 .mcp.json（client 模式）」）；正文两块，各带小节引导行——「离线为主（默认推荐）」→ `--cache` 片段；「在线为主」→ http 片段。**http 块的 Bearer 固定渲染占位 `<server 机 Projects 页签发的 token>`**——client 机从不持有 project token（设备码只用于拉缓存，两道闸门模型），note 指明获取处：server 机主控台 Projects 页 `[a]` 新增 / `[e]` 轮换签发。note 必含：

- `"type": "http"` **必填**——漏了 Claude Code 会当 stdio 处理并拒绝该条目；
- 两种形态用的是**同一个 project token**；
...
| 文件 | 断言 |
|---|---|
| `tokenview_test.go`（新增） | `body()`：裸 token 恒在首位；usage/recovery/snippet 零值 → 输出仅 token（设备码/向导兼容锚）；三字段齐 → 分块顺序 token→用途→丢失→片段；snippet 非空时含引导语首行与「说明：」块（归属锚） |
...
| `wizardsteps_test.go` | `mcpHttpConfigLines`：`"type": "http"` / url / Bearer 代入；尾成员无逗号（空 notes 也合法）；**转义锚**——含 `"` 与 `\` 的构造 URL 渲染产物 `json.Unmarshal` 合法；与 `mcpConfigLines` 共享骨架不回归（现有用例不动） |
...
3. **逐工具语义**：
   ...
   - `upload_file`：本地文件**或目录**（递归）；上传前确认远端目标路径（preflight 拒绝语义照实写：写实现时以 `internal/mcpserver` 实际拒绝条件为准）。
...
| `multi-machine.md` | 「离线只读缓存」Step 2 双形态片段处互链两篇 TUI 教程；该节标注「片段权威源 = 代码渲染器（`mcpConfigLines`/`mcpHttpConfigLines`），文档片段如与之不符以代码为准」（R3 缓解） |
...
5. **片段一致性（口径钉死）**：以**固定占位值**渲染——stdio 块用 token 占位 `<TOKEN>`、http 块用 `url=https://192.0.2.5:7878/` + token 占位 `<TOKEN>`——渲染产物与文档片段**逐字节一致**（plan 阶段临时 `go run` 渲染比对，人工核对后固化进文档）。secretView 内嵌的真 token 不参与本比对（它是运行期用户值）。
...
- **R3 — 文档多处片段漂移**：stdio/http 片段将存在于「代码渲染 + 3 篇新文档 + README/multi-machine.md」。缓解：验收 5 的占位值逐字节比对 + 源头只有 `mcpConfigLines`/`mcpHttpConfigLines` 两个渲染器；multi-machine.md 联动行显式声明「以代码渲染为准」。
- **R4 — `clientFinishScreen(serveURL)` 空值**：nil/空防御职责在**两处调用点**（`m.cred` 判空后传参，杜绝 nil 解引用）；函数内对空串渲染 `<serve URL>` 占位。测试覆盖（§4.3）。
- **R5 — serveURL 用户输入的极端形态**：含引号/反斜杠的 URL 由 `mcpHttpConfigLines` 的值转义兜住（§4.2，实验证实拼接不健壮）；URL 尾斜杠差异已被实验证实**功能无关**（§3 事实表），不做归一化。
