...
- 日期：2026-08-20
- 状态：rev3（定稿）。rev1 消化首轮反馈（尾斜杠实验结论、JSON 转义硬要求、nil 防御职责、tokenRef 占位等）；rev2 补规格自洽性（渲染机制零改动路径、`encoding/json` 值编码、双形态片段、golden fixture、事实核对表）；rev3 按第三轮反馈清单修订（owner 授权）：值编码钉死 `SetEscapeHTML(false)`、nil 调用点流程断言、docs-vs-golden 自动比对、`<serve URL>` 中立占位、stdio 形态写入手册环境差异、G3/G4 教程事实核对纪律、比对范围补全、单机忽略注。
...
// http 块的 URL 用中立占位 "<serve URL>"（默认形态 https://<主机>:7878/
// 写进 note）——serve 的地址/端口随 --addr 而定、明文部署亦存在，broker
// 机不可靠知道自己的对外 URL；token 是照抄即用的关键值，必须为真值。
...
// using a json.Encoder with SetEscapeHTML(false) applied to the COMPLETE
// value string (e.g. "Bearer "+token), then the outer quotes are stripped
// into the template. TWO defaults are pinned, both load-bearing:
//   1) strconv.Quote-family is FORBIDDEN — it emits Go-only escapes
//      (\a \v) that are illegal JSON (RFC 8259) for control characters.
//   2) SetEscapeHTML(false) is REQUIRED — json.Marshal/Encoder default
//      HTML-escaping emits backslash-u escapes for < > & (U+003C/003E/0026,
//      six visible chars instead of one); the snippet's placeholders are
//      full of angle brackets (<TOKEN>, <serve URL>), and escaped output
//      would wreck copy-paste readability and diverge from every
//      hand-written doc example.
// Same helper applies symmetrically to the stdio builder's token value.
...
| `wizardsteps_test.go` | `mcpHttpConfigLines`：`"type": "http"` / url / Bearer 代入；尾成员无逗号（空 notes 也合法）；**编码锚**——对 **URL、stdio env token、http Bearer 三个插值点**各构造含 `"`、`\`、控制字符（`\x07`、`\v`）、`&`、`<` 的值，渲染产物 `json.Unmarshal` 合法**且不含 HTML 转义形态**（SetEscapeHTML(false) 锚）；**golden fixture**——以固定占位值（stdio：token=`<TOKEN>`；http：`url=https://192.0.2.5:7878/` + token=`<TOKEN>`）渲染的两块产物与内嵌 golden 串**逐字节**比对（文档片段以该 golden 为准固化——R3 自动回归锚）；与 `mcpConfigLines` 共享骨架不回归（现有用例不动） |
| `wizardsteps_docsync_test.go`（新增） | **docs-vs-golden 自动比对**：读取 README + docs/ 各含 `mcpServers` JSON 块的文档（见 §5.5 范围清单），抽出 JSON 块与以占位值渲染的 golden 逐字节比对——文档侧漂移即测试红（不再依赖一次性人工核对） |
| `clientpage_test.go` | `clientFinishScreen("https://192.0.2.5:7878")`：同时含 `--cache` 片段、`"type": "http"`、serveURL、**Bearer 占位行 `<server 机 Projects 页签发的 token>`**、「必填」note；`clientFinishScreen("")` → `<serve URL>` 占位不 panic；**调用点锚（R4 流程级）**——pull 成功链路的真实调用路径在 `m.cred == nil` / `URL == ""` 时传 `""` 并渲染占位、不 panic（经 clientModel 流程断言，非仅函数级——防"防御下放调用方却未测调用方"） |
...
7. **事实核对纪律（硬要求，与 §4.6 手册同款）**：本篇所有键位、向导问句原话、表单校验规则、「继续添加？」循环语义、`!` 过滤规则等断言，成文时逐条对照 `internal/tui` 源码核实，以代码为准——教程最易被逐字照抄执行，错一个键名即直接翻车。
...
8. **事实核对纪律（硬要求，与 §4.6 手册同款）**：client 面板 `s`/`c`/`t` 键义、页头摘要字段、10s 同步超时、失败保留旧缓存、设备码掩码不预填（留空=不变）、TTL 行为等断言，成文时逐条对照 `internal/tui` 源码核实，以代码为准。
...
5. **环境差异（三态）**：单机/stdio（本机 broker 子进程，**可写**）｜在线 serve（远程 broker，可写）｜离线 cache（只读快照）——工具面对三种形态完全一致，你通常无需分辨；遇到 `ErrReadOnly`/read-only 报错 = 当前挂在只读缓存上 → 报告 owner 切到在线/本机形态，**别**重试写操作。
...
5. **片段一致性（口径钉死）**：比对对象 = **仅 JSON 块**（不含引导语与「说明：」注记）；比对基准 = §4.3 golden fixture 以固定占位值（stdio：token=`<TOKEN>`；http：`url=https://192.0.2.5:7878/` + token=`<TOKEN>`）的渲染产物；比对范围 = **3 篇新文档 + README + multi-machine.md（含 serve 节 Step 3 示例）+ quickstart ×2 + getting-started.md + agent-access.md**（后四篇正文现有 `.mcp.json` JSON 块一并纳入——已核实四篇均含 `"mcpServers"` 块）；比对机制 = **golden 内嵌 `wizardsteps_test.go` + `wizardsteps_docsync_test.go` 自动读文档比对**（文档侧漂移即测试红，不依赖一次性人工核对）；首次固化用临时 `go run` 打印 golden 串人工复核后写入文档。secretView 内嵌的真 token 不参与比对（运行期用户值）。
6. **agent-tools.md 事实核对表完成**：文末「行为依据」小节落地——每条数值/行为断言一行实现锚点（文件:行号 或 文档节名），全部对照 `internal/mcpserver` / 既有文档核实。
7. **两篇 TUI 教程事实核对完成**：键位/问句原话/数值断言逐条对照 `internal/tui` 源码核实（§4.4-7 / §4.5-8 纪律的验收面）。
8. 合并时按仓库惯例 bump 版本号（建议 v0.8.10，merge 时 owner 可改）。
...
- **R3 — 文档多处片段漂移**：片段存在于「代码渲染 + 3 篇新文档 + README + multi-machine.md + quickstart ×2 + getting-started + agent-access」。缓解（三层）：golden fixture 快照测试（渲染器改动即红）+ `wizardsteps_docsync_test.go` 自动读文档比对（文档漂移即红）+ multi-machine.md 联动行显式声明「以代码为准」。
...
- 顺序：代码（§4.1→4.2→4.3）→ 文档三篇（§4.4-4.6；agent-tools.md 的行为断言与两篇 TUI 教程的键位/数值断言**均先做事实核对再成文**）→ 联动（§4.7）→ 验收（§5）。
