...
- **信号 4 正向腿的种法受令牌能力约束（Plan 38 T1 实现期发现，2026-08-26 探针）**：种非白名单 owner 需要 WRITE_OWNER，而硬化 DACL 刻意不给当前用户（仅管理员可重属主）；且非提权（UAC 过滤）令牌下连 fresh 文件的 owner 指派都被拒——Everyone / BUILTIN\Users → `ERROR_INVALID_OWNER`，Administrators（deny-only）同样被拒，硬化文件上改回 self 也 `ACCESS_DENIED`（owner 隐式权只有 READ_CONTROL/WRITE_DAC）。因此 `TestInspectFileACL_Owner` 采 ① 腿同款降级模式：可种环境（如提权令牌）全跑，不可种环境带因 Skip；白名单 owner 的负向覆盖由 `TestInspectFileACL_Hardened` 的 creator-owner 断言兜底。不宣称信号 4 在非提权环境有端到端覆盖。
- **条件允许（callback-allow）ACE 纳入 walk；object-型 granting ACE 未纳入（Plan 38 终审修复登记，2026-08-26）**：grantor walk 现纳入 `ACCESS_ALLOWED_CALLBACK_ACE_TYPE`（0x9，条件允许）——它与 `ACCESS_ALLOWED_ACE` 同前缀布局 Header|Mask|SidStart（条件求值数据在完整 SID 之后，MS-DTYP §2.4.4.6），现有 `*ACCESS_ALLOWED_ACE` cast 对其健全。object-型 granting ACE（`ACCESS_ALLOWED_OBJECT_ACE_TYPE` 0x5、`ACCESS_ALLOWED_CALLBACK_OBJECT_ACE_TYPE` 0xB）及过时的 compound allow（0x4）因 SID 偏移不同（Flags+可选 GUID 前置于 SidStart）**未纳入 walk**——若此类 ACE 单独授予非白名单主体读权限，doctor 漏报，登记为 fail-open 残余。该类 ACE 无法经 `SetEntriesInAcl`/`ACLFromEntries` 种入（EXPLICIT_ACCESS 只产 plain allow/deny），测试矩阵不可覆盖，以 `isWalkedAllowAceType` 表驱动单测 + 代码审查钉。
- **GetAce 单 ACE 读失败静默跳过为 fail-open 残余（Plan 38 终审登记，2026-08-26）**：walk 中 `GetAce` 失败仅 `continue`——该路径实际不可达（DACL 来自 `GetNamedSecurityInfo` 刚返回的 SD，同一进程对其顺序读不应失败），故按 fail-open 而非 fail-closed 处理；畸形 SID 的 `String()` 错误被吞掉理论上可产出空串进 `UnexpectedReadGrantors`（同为 API 层已验证过的 SD 上的不可达路径）。两处均登记：结构信号（DaclNull/Protected/owner）不受影响，仅极端畸形 SD 下可能漏报个别 grantor。
