...
- **null-DACL 强制测试未决**（§4.1 ①）：nil-dacl 种法探针留作实现期 TDD 首步；若证实不可种，信号 1 的覆盖降级为解析层构造单测 + 代码审查，登记不宣称端到端；探针结论按 §5 回填，不留悬空。
