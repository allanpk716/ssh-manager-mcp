...
// TestClassifyPullError403Unbound (Plan 39): the unbound-device-code refusal
// (DoPull surfaces the serve's "not bound to a profile" body) classifies as its
// OWN banner (设备码未绑定 profile), not 设备码无效 — the code is valid; the
// binding is the owner's server-side fix. Precision (code-review #6): a bare
// 403 without the discriminator (proxy/WAF/fail-closed) and bare "403"
// substrings (fingerprint hex, ports) must NOT trigger.
func TestClassifyPullError403Unbound(t *testing.T) {
	for _, raw := range []string{
		"pull: server returned 403 — device code not bound to a profile (owner: run ssh-manager cache-tokens bind on the server)",
	} {
		got := classifyPullError(errors.New(raw))
		if !strings.Contains(got, "设备码未绑定 profile") {
			t.Fatalf("403-unbound 未分类: %q → %q", raw, got)
		}
		if strings.Contains(got, "设备码无效") {
			t.Fatalf("403 不得分类为设备码无效: %q → %q", raw, got)
		}
	}
	// 负例①：无判别词的 403（代理/WAF/serve fail-closed 门）——不得归入未绑定。
	for _, s := range []string{
		"pull: server returned 403 (forbidden)",
		"pull: server returned 403 — 403 Forbidden: IP not allowed",
	} {
		if got := classifyPullError(errors.New(s)); strings.Contains(got, "设备码未绑定") {
			t.Fatalf("无判别词的 403 误分类: %q → %q", s, got)
		}
	}
	// 负例②：指纹 hex 含 "403" 子串 / 端口 4031。
	for _, s := range []string{
		"dial tcp: 4031 connection refused",
		"pin sha256:bb4033... mismatch",
	} {
		if got := classifyPullError(errors.New(s)); strings.Contains(got, "设备码未绑定") {
			t.Fatalf("非 403 误分类: %q → %q", s, got)
		}
	}
}
