...
| **T12** relay-resume (Plan 47) | Plan 47 tool | `relay_file` a 48 MiB fixture gpu → web whose destination ships a PRE-SEEDED interrupted state (chunk-0 partial + manifest, per-M-run fresh randomness) → RESUME (resumed_chunks ≥ 1) → engine "relay done:" via exec_output → closed loop (sha256sum on both servers, digests equal) | **not yet recorded** (gated per-task, T9 precedent — the real M=5 run is an owner/CI gate) | deterministic (`scoreT12`: relay_file on the exact fixture paths + resume evidence (result JSON `resumed_chunks ≥ 1` or plan line `(resumed N≥1)`) + "relay done:" in exec_output + matching src/dst sha256 pair + no-leak; a fresh=true restart or a `cp` side-channel fails the resume/surface assertions by construction) |
...
| `TestEvalT12RelayFile` | double-gated (`requireEval`) | **yes** (M=5) | the Plan-47 relay capability task — interrupted-transfer resume (test-side pre-seeded chunk-0 partial + manifest, `SSHMGR_TRANSFER_CHUNK=16MiB` pinned in the mcp.json env so the grid matches) + engine completion + remote sha256 closed loop, scored by `scoreT12` (deterministic; NOT in the §12.3 gate — T9 precedent) |
| `TestScoreT12RelayResume` | always-on | no | `scoreT12`'s branches (pure synthetic transcripts): pass shape + fresh-restart fails the resume assertion + non-relay side-channel fails + digest mismatch fails + the <2-seed driver-bug guard |
| `TestBrokerToolsRelayFile` | always-on | no | `relay_file` is in `mcpserver.BrokerTools` — pins scoreT6/scoreT8's auto-extended zero-tolerance surface (anti-slice-drift, Plan 47 mirror of the trio/upload_content rows) |
...
created if missing. **§6 cap: 1 MiB is a hard per-file bound** — a single
larger file is REFUSED before transfer (error names file/size/cap, zero bytes
sent; in a directory upload, files completed before the refusal remain); when
multiple files each stay within the cap but their cumulative total crosses
1 MiB, already-completed files are kept, `truncated=true` is honestly reported,
and later files are not uploaded — retry smaller. **Profile-gated**
